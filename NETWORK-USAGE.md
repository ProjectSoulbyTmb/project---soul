...
| User-opened HTTPS page in the browse or Discord **guest overlay** | User opens an overlay and navigates | URL request, IP, site cookies in an isolated Electron partition (`persist:eidovara-guest-browse` / `persist:eidovara-guest-discord`). Workspace renderer stays locked. Discord tokens are not sent to Soul or Assist. |
