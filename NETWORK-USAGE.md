...
| `github.com/ProjectSoulbyTmb/project---soul` | After 18+: Settings/companion **Check for updates**, or automatic GitHub Releases checks if you turn that setting on (default off); user-approved installer download | App version through user agent, IP address; installer request after checksum metadata (`latest.yml` SHA-512 and/or `update.json` SHA-256). Builds are Authenticode-unsigned. |
