...
| Optional Eidovara service `/health`, `/v1/config`, `/v1/status` | After 18+ confirmation: launch check, Settings **Connect**, or local admin **Test service**. Default base is `https://api.eidovara.org`; paste another HTTPS base to override | No conversations or payment data; health/config/status JSON only |
