...
| GitHub official release channel | After 18+: manual Check for updates, or automatic checks if enabled (default off); user-approved installer download | App version via user agent, IP address; installer bytes after checksum verification. Builds are Authenticode-unsigned. |
