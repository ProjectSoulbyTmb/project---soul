...
- Final published Windows release: `Eidovara-0.22.2-Windows-x64-Setup.exe`, 106,691,524 bytes (about 101.75 MiB), SHA-256 `F29A52F0495AB111A277780706E75ED616B6C236E25C3BDDF36E144ED5326675`. The launcher, desktop program, website, service metadata, updater, and current documentation all identify v0.22.2. The installer is Authenticode-unsigned.

- Public site restyle plus version-honest pages: larger hero (one filled CTA and a quiet product link), three benefits instead of a five-card grid, quieter plans, and a `nav-cta` Download control. Every public `docs/*.html` page and the live advertised installer identify **v0.22.2**. The current Setup.exe is advertised with its measured size and SHA-256.

- Pre-publication two-track stage: source had reached **v0.22.2** while the advertised installer was still v0.19.1. That temporary state ended when the measured v0.22.2 Setup.exe was published; historical tags were not moved.
...
- Gaming/productivity workspace overlays: floating Eidovara windows for Soul chat, HTTPS browsing, and Discordâ€™s own site in a sandboxed guest session (`persist:eidovara-guest` / `persist:eidovara-guest-discord`). Not an official Discord overlay, not game-process injection, no OS-wide hotkeys. Play desk lists confirm-launched apps plus this processâ€™s CPU/memory. Companion chips include `open-chat-overlay`, `open-browse-overlay`, `open-discord-overlay`, and `set-always-on-top`. Ctrl+Shift+O / Ctrl+K open them. Adult Mode closes guest overlays. Assist stays opt-in and does not receive Discord tokens or guest DOM. Workspace CSP is unchanged (`media-src https: eidovara-media:`, never `'self'`). Guest navigations with `javascript:`, `data:`, `blob:`, or `vbscript:` are refused.
...
- New unsigned Windows installer: `package.json` / Worker health / website-helper version is `0.19.1`. App id stays `com.soulconsciousnessstudios.eidovara`. Tags `v0.18.0`â€“`v0.19.0` already exist and were not moved.
...
- Shared `docs/tokens.css` / renderer tokens honor `prefers-reduced-motion`. Home, Download, and Help advertise v0.19.0. Honest 18+ / unsigned / no-payments / Assistâ‰ Soul remain visible. No neural TTS/VRM/OBS, no payments, no `workers.dev` default, no `media-src 'self'`.
...
- Desktop Settings â†’ Eidovara service defaults to the official custom hostname `https://api.eidovara.org` (HTTPS base only, no path). Empty/default still resolves there; paste another HTTPS base to override. No `workers.dev` host is compiled in. Conversations are not sent. Fetch failure stays local-first.
- First-party IP notices: LICENSE/EULA close relicensing-as-OSS and contributor-mark loopholes; SPDX `LicenseRef-Eidovara-Source-Available-1.0` on LICENSE and first-party `src/**/*.js` / `docs/*.js`; Soul Consciousness Studios stays the intended publisher only; assignment files remain unsigned templates. No Â®, patent, or Copyright Office registration is claimed.
...
- First-party IP ownership: OWNERSHIP.md honest limits; unsigned contributor and entity assignment templates; LICENSE/EULA/NOTICE/TERMS reserve qualifying original Eidovara expression to Soul Consciousness Studios without claiming Electron, Chromium, Node.js, Windows, Wikimedia, or user content; marks stay unregistered â„¢; no fake copyright-office, USPTO, or patent claims. Source-available evaluation license unchanged (not MIT/Apache/GPL). GitHub PRs remain unaccepted until a privately executed written agreement.
...
- Renamed the application to the preliminarily screened coined mark Eidovaraâ„¢ and identified Soul Consciousness Studiosâ„¢ as Soul Consciousness Studios's intended business name.
