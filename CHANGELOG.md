...
- Public online path: the GitHub Pages homepage now explains visit → unsigned Windows download (or build from source) → local desktop app; optional Worker is `/health` and `/v1/config` only. Owner runbook covers Pages, Releases, wrangler, and an optional custom domain. `Release Windows` accepts `workflow_dispatch` for unsigned artifacts and still publishes only on `v*` tags. No Authenticode, no live checkout, no `workers.dev` default in the app.
