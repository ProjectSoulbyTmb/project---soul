...
- Windows-first chrome on the 0.19.0 kernel: tray (show / always-on-top / quit), always-on-top, open-at-login (Windows via Electron), in-app Now Playing from local `eidovara-media:` files, recents, an in-app notice drawer, a media sleep timer that pauses only this player, and a local Ctrl+K calculator (`12 + 7`, `12 ft in m`) with no live FX. No Windows Recall, no DRM rip, no Spotify/VLC/iTunes injection, no global hotkeys into other apps.
- Published SHA-256 for the real unsigned `Eidovara-0.19.0-Windows-x64-Setup.exe` on GitHub Releases: `F2B0D9BB0A887294CF58A43C75DF67FA422C2120540DE03D5227A9B239D08310` (~101.3 MiB). The 0.18.2 hash is no longer advertised as 0.19.0.
...
- Primary public download is `https://github.com/ProjectSoulbyTmb/project---soul/releases/latest/download/Eidovara-0.19.0-Windows-x64-Setup.exe` (the NSIS `.exe`, about 101.3 MiB, not the source repository). Pinned tag asset: `https://github.com/ProjectSoulbyTmb/project---soul/releases/download/v0.19.0/Eidovara-0.19.0-Windows-x64-Setup.exe`. Authenticode-unsigned; not Microsoft-certified. SHA-256 `F2B0D9BB0A887294CF58A43C75DF67FA422C2120540DE03D5227A9B239D08310` (published `SHA256SUMS.txt` on the v0.19.0 GitHub Release). GitHub/Sigstore provenance is not Authenticode.
