...
- Persistent companion dock after 18+: heartbeat, presence, module chips, custom actions, a local Ask Soul composer, hold-to-talk on Windows `SpeechRecognition`, and Ctrl+/ focus. Phrasing knobs change local wording only at high values and never claim sentience. Avatar click opens the companion instead of hiding it. Dashboard composer can ask the Worker helper for one send when opt-in is on. Looks include ribbon/hidden plus CSS/canvas/local image; not VRM/MakeHuman.
