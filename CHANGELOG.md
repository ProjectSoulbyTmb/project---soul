...
- **v0.19.1** (source on this branch; GitHub tag after merge). Automatic application upgrader: `electron-updater` 6.8.9 reads GitHub Releases `latest.yml`, verifies SHA-512, Settings **Check GitHub for updates automatically**, prompt before download and again before install, Defender scan, `quitAndInstall`. Does **not** download an arbitrary `.exe` and `shell.openPath` it. NSIS `SetOverwrite on` + nsProcess close/kill. Does not move tags `v0.18.0`–`v0.19.0`.
...
- **Live advertised download remains v0.19.0** until a real `Eidovara-0.19.1-Windows-x64-Setup.exe` exists. Independently hashed: `Eidovara-0.19.0-Windows-x64-Setup.exe` SHA-256 `F2B0D9BB0A887294CF58A43C75DF67FA422C2120540DE03D5227A9B239D08310` (106263840 bytes). The previous advertised `EF228574…` hash is the **v0.18.2** installer, not 0.19.0.
