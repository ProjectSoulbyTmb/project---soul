...
- **v0.19.2** (source on this branch; GitHub tag after merge). Automatic application upgrader: `electron-updater` 6.8.9 reads GitHub Releases `latest.yml`, verifies SHA-512, Settings **Check GitHub for updates automatically**, prompt before download and again before install, Defender scan, `quitAndInstall`. Does **not** download an arbitrary `.exe` and `shell.openPath` it. NSIS `SetOverwrite on` + nsProcess close/kill. Does not move tags `v0.18.0`–`v0.19.1`.
- **Live advertised download remains v0.19.1** until a real `Eidovara-0.19.2-Windows-x64-Setup.exe` exists. Independently hashed here: `Eidovara-0.19.1-Windows-x64-Setup.exe` SHA-256 `72F4D09ADA17593F0391438A5375ABC9351041DA8ABB252E68271B8FDACCA7D8` (106277155 bytes). The previous advertised `F2B0D9BB…` hash is the **v0.19.0** installer, not 0.19.1.
...
- Primary public download is `https://github.com/ProjectSoulbyTmb/project---soul/releases/latest/download/Eidovara-0.19.1-Windows-x64-Setup.exe` (the NSIS `.exe`, about 101.3 MiB, not the source repository). Pinned tag asset: `https://github.com/ProjectSoulbyTmb/project---soul/releases/download/v0.19.1/Eidovara-0.19.1-Windows-x64-Setup.exe`. Independently hashed SHA-256 `72F4D09ADA17593F0391438A5375ABC9351041DA8ABB252E68271B8FDACCA7D8` (106277155 bytes). Authenticode-unsigned; not Microsoft-certified. GitHub/Sigstore provenance is not Authenticode.
...
- Primary public download is `https://github.com/ProjectSoulbyTmb/project---soul/releases/latest/download/Eidovara-0.19.0-Windows-x64-Setup.exe` (the NSIS `.exe`, not the source repository). Pinned tag asset: `https://github.com/ProjectSoulbyTmb/project---soul/releases/download/v0.19.0/Eidovara-0.19.0-Windows-x64-Setup.exe`. Authenticode-unsigned; not Microsoft-certified. SHA-256 `F2B0D9BB0A887294CF58A43C75DF67FA422C2120540DE03D5227A9B239D08310` (106263840 bytes).
