...
- Workspace renderer opened to HTTPS: CSP `connect-src https:` (not `*`, not `'none'`) so first-party workspace JS may fetch/XHR/WebSocket to HTTPS. Optional in-app browsing is an isolated `WebContentsView` on the Web desk (`persist:eidovara-web`, sandboxed, no Soul preload). The main window still refuses to navigate `index.html` off-site and still denies `<webview>`. `media-src` stays `https: eidovara-media:` (never `'self'`). Entertainment Spotify/YouTube buttons stay official HTTPS search + system browser, not in-process embeds. Adult Mode and a missing 18+ gate destroy the web view. User-directed; not a background crawler.
