...
Operator override example (not a `workers.dev` bake): see [docs/PAYMENTS_AND_SITE.md](docs/PAYMENTS_AND_SITE.md). Never commit `CLOUDFLARE_API_TOKEN`.
...
2. **Redeploy Worker if `/health` drifts** — After `npx wrangler deploy` from `server/`, the desktop already defaults to `https://api.eidovara.org`. Do not compile a `workers.dev` host. Owner can still paste another HTTPS base in Settings to override.
