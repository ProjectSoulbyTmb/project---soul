...
| Cloudflare Worker | Wrangler name `eidovara-api`. Public GET `/health`, `/v1/config`, `/v1/status`; GET/POST `/v1/assist`. Fail-closed otherwise. Payments stay off. Redeploy with `npx wrangler deploy` from `server/` after Worker `version` changes. Health JSON version is `0.18.2`. |
| Windows installer | Tag `v0.18.2` publishes unsigned `Eidovara-0.18.2-Windows-x64-Setup.exe`. Primary site CTA is `https://github.com/ProjectSoulbyTmb/project---soul/releases/latest/download/Eidovara-0.18.2-Windows-x64-Setup.exe`. SHA-256 `EF228574DCDF34B8A9039654F2B762FAB6D289CCA9A94B2ECCF048AE971FE711`. Authenticode-unsigned; GitHub/Sigstore provenance is not Authenticode. Tags `v0.18.0` and `v0.18.1` already exist and must not be force-moved. |
...
2. **Paste Worker base / optional custom domain** — After `npx wrangler deploy` from `server/`, paste the HTTPS Worker **base** into Settings → Eidovara service. Optional Cloudflare / GitHub Pages custom domain; still do not hard-code the API host in the app.
