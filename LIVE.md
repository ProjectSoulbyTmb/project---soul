...
| Surface | Status |
| --- | --- |
| `main` | Live tagged product is `v0.19.0` (`c7d67337b44ce97de541db4deb656931dae45dee`). Source on this branch is `0.19.1` (electron-updater + NSIS overwrite). Do not move `v0.18.3` or `v0.19.0`. |
...
| Cloudflare Worker | Wrangler name `eidovara-api`. Public GET `/health`, `/v1/config`, `/v1/status`; GET/POST `/v1/assist`. Fail-closed otherwise. Payments stay off. Custom hostname `https://api.eidovara.org`. Redeploy with `npx wrangler deploy` from `server/` after Worker `version` changes. Source health JSON version on this branch is `0.19.1` (live Worker stays `0.19.0` until owner deploys). `WEBSITE_URL` is `https://eidovara.org/`. |
| Windows installer | Tag `v0.19.0` publishes unsigned `Eidovara-0.19.0-Windows-x64-Setup.exe` (106263840 bytes). Primary site CTA is `https://github.com/ProjectSoulbyTmb/project---soul/releases/latest/download/Eidovara-0.19.0-Windows-x64-Setup.exe`. Pinned tag asset: `https://github.com/ProjectSoulbyTmb/project---soul/releases/download/v0.19.0/Eidovara-0.19.0-Windows-x64-Setup.exe`. Independently hashed SHA-256 `F2B0D9BB0A887294CF58A43C75DF67FA422C2120540DE03D5227A9B239D08310`. Authenticode-unsigned; GitHub/Sigstore provenance is not Authenticode. Tags `v0.18.0`, `v0.18.1`, `v0.18.2`, `v0.18.3`, and `v0.19.0` already exist and must not be force-moved. Next updater ship is `v0.19.1` after merge. |
...
Cloudflare Pages project `eidovara` serves the same `docs/` IA as GitHub Pages. After `npx wrangler pages deploy docs --project-name=eidovara`, these should return HTTP 200 with the v0.19.0 product surface (not the retired Ask Soul homepage):
