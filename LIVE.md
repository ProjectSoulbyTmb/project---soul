...
| Windows installer | Tag `v1.0.0` publishes unsigned `Eidovara-v1.0.0-Windows-x64-Setup.exe`; its measured size and SHA-256 land in the release’s SHA256SUMS.txt and latest.yml when CI builds it.; no digest is published on this page until CI measures one. Authenticode-unsigned; GitHub/Sigstore provenance is not Authenticode. |
