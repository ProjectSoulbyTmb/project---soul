...
| Cloudflare Worker | Wrangler name `eidovara-api`. Public GET `/health`, `/v1/config`, `/v1/status`; GET/POST `/v1/assist`. Fail-closed otherwise. Payments stay off. Health/config/status source version and live installer version are `0.22.2`. |
| Windows installer | Tag `v0.22.2` publishes unsigned `Eidovara-0.22.2-Windows-x64-Setup.exe` (106,691,524 bytes, about 101.75 MiB). SHA-256 `F29A52F0495AB111A277780706E75ED616B6C236E25C3BDDF36E144ED5326675`. Authenticode-unsigned; GitHub/Sigstore provenance is not Authenticode. |
| Edition | v0.22.2 is a full free Alpha. Currently implemented features are not blocked by paid entitlement; live checkout/subscription processing remains off. |
...
Public release facts that must remain consistent across Home, Product, Download, Status, FAQ/help, helper knowledge, Worker/service payloads, release metadata, and tests:
...
- Current payloads must identify release/source `0.22.2` and the final v0.22.2 installer facts above.
...
Older releases may remain in `CHANGELOG.md`, old GitHub tags/releases, or immutable git history as historical evidence. No current-facing website, README, live-state, helper, Worker/service, installer, updater, or release-policy statement should identify v0.19.x as the current product. The current release is v0.22.2.