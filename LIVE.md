...
3. **www.eidovara.org DNS** — Apex `eidovara.org` is already a Cloudflare Pages custom domain. `www` does not resolve. Owner click: Cloudflare Dashboard → Pages → `eidovara` → Custom domains → add `www.eidovara.org`, and DNS CNAME `www` → `eidovara.pages.dev` (proxied). Do not point GitHub Pages at this hostname (no `docs/CNAME`).
