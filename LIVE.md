...
| Surface | Status |
| --- | --- |
| `main` | PR #10 (`cursor/engine-product-surface-c180`) merged. Merge commit: `394cf3a287fa6fc665aed7568201e93df4165884`. Current product version on this snapshot is `0.19.0`. |
...
| Cloudflare Worker | Wrangler name `eidovara-api`. Public GET `/health`, `/v1/config`, `/v1/status`; GET/POST `/v1/assist`. Fail-closed otherwise. Payments stay off. Custom hostname `https://api.eidovara.org`. Redeploy with `npx wrangler deploy` from `server/` after Worker `version` changes. Health JSON version is `0.19.0`. `WEBSITE_URL` is `https://eidovara.org/`. |
| Windows installer | Tag `v0.19.0` publishes unsigned `Eidovara-0.19.0-Windows-x64-Setup.exe` (~101.3 MiB). Primary site CTA is `https://github.com/ProjectSoulbyTmb/project---soul/releases/latest/download/Eidovara-0.19.0-Windows-x64-Setup.exe`. Pinned tag asset: `https://github.com/ProjectSoulbyTmb/project---soul/releases/download/v0.19.0/Eidovara-0.19.0-Windows-x64-Setup.exe`. SHA-256 `EF228574DCDF34B8A9039654F2B762FAB6D289CCA9A94B2ECCF048AE971FE711` until the v0.19.0 Release asset is hashed (do not use this as a claim that the 0.18.2 file is 0.19.0). Authenticode-unsigned; GitHub/Sigstore provenance is not Authenticode. Tags `v0.18.0`, `v0.18.1`, `v0.18.2`, and `v0.18.3` already exist and must not be force-moved. |
