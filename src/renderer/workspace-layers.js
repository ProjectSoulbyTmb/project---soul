...
(() => {
  ...
  function renderFocusBar() {
    ...
    if ($('#focusQuietLabel'))
      $('#focusQuietLabel').textContent = focus.label || t('focusQuiet', 'Focus session');
    if (focus.active === true && remain <= 0) {
      if (!focusStopping) {
        focusStopping = true;
        Promise.resolve(window.eidovaraLayers?.stopFocus?.()).finally(() => {
          focusStopping = false;
        });
      }
      return;
  ...
  function tileDefs(ctx) {
    ...
    return {
      focus: {
        ...
        value:
          focus.active && remain > 0
            ? formatRemain(remain)
            : state.assistant?.preferences?.focusMode || 'general',
      ...
      research: {
        ...
        run: () => {
          ctx.setView('chat');
          ctx.send('Search the internet for current information I need.');
        },
      },
      memory: {
  ...
  function renderDashboard(ctx) {
    ...
    for (const id of visible) {
      ...
      unpin.addEventListener('click', e => {
        e.preventDefault();
        e.stopPropagation();
        window.eidovaraLayers.unpin(id);
      });
      ...
      up.addEventListener('click', e => {
        e.preventDefault();
        e.stopPropagation();
        window.eidovaraLayers.move(id, -1);
      });
    ...
    if (hiddenIds.length) {
      ...
      extra.append(
        Object.assign(document.createElement('small'), {
          textContent: t('pinWidget', 'Pin a tile'),
        })
      );
    ...
    roles.append(
      ...
      Object.assign(document.createElement('strong'), {
        textContent: setup.length ? setup.join(', ') : t('notConfigured', 'not configured'),
      }),
      Object.assign(document.createElement('span'), {
        className: 'next',
        textContent: t('nextConfigure', 'Configure roles'),
      })
    );
  ...
  function paintPalette(selected) {
    ...
    if (!paletteItems.length) {
      box.append(
        Object.assign(document.createElement('li'), {
          textContent: t('paletteEmpty', 'No local matches. Search stays on this PC.'),
        })
      );
      return;
    }
    paletteItems.forEach((item, index) => {
      ...
      b.append(
        Object.assign(document.createElement('strong'), {
          textContent: item.title || item.label || item.id,
        })
      );
      b.append(
        Object.assign(document.createElement('small'), {
          textContent: `${item.kind || 'command'}${item.summary ? ' · ' + item.summary : ''}`,
        })
      );
  ...
  async function queryPalette(text) {
    ...
    try {
      paletteItems =
        (await window.soul.workspace('palette', { query: text, includeApps: true })) || [];
    } catch {
      paletteItems = [];
    }
    paintPalette(0);
  ...
  async function runPaletteItem(item) {
    ...
    if (item?.id && window.soul?.workspace) {
      try {
        await window.soul.workspace('recent', {
          id: item.id,
          title: item.title,
          kind: item.kind,
          action: item.action,
  ...
  async function executeAction(action) {
    ...
    if (type === 'open-view' && action.view && typeof window.eidovaraSetView === 'function') {
      ...
      if (action.panel)
        $(`#${action.panel}`)?.scrollIntoView({
          behavior: reducedMotion() ? 'auto' : 'smooth',
          block: 'center',
        });
    }
    if (type === 'open-setup' && typeof window.eidovaraOpenSetup === 'function')
      window.eidovaraOpenSetup(true);
    ...
    if (type === 'open-legal' && typeof window.eidovaraShowLegal === 'function')
      window.eidovaraShowLegal(action.legal || 'about');
    ...
    if (type === 'run-command' && action.command && typeof window.eidovaraSend === 'function') {
      if (typeof window.eidovaraSetView === 'function')
        window.eidovaraSetView(action.view || 'chat');
      window.eidovaraSend(action.command);
    }
    if (type === 'confirm-launch-app' && action.appId && window.soul?.launchApplication) {
      try {
        await window.soul.launchApplication(action.appId);
      } catch (err) {
        alert(String(err?.message || err));
      }
    }
    if (
      typeof window.eidovaraRunAction === 'function' &&
      [
        'open-overlay',
        'open-chat-overlay',
        'open-browse-overlay',
  ...
  async function startFocus(minutes, label) {
    if (!window.soul?.workspace) return;
    const kernel = await window.soul.workspace('start-focus', {
      minutes: minutes || 25,
      label: label || t('focusQuiet', 'Focus session'),
    });
    window.eidovaraKernel = kernel;
    if (window.eidovaraState)
      window.eidovaraState.kernel = {
    ...
    const rows = [
      {
        ...
        detail: t(
          'shortcutPalette',
          'Command palette — jump to views, intents, settings, legal, modules, and linked apps. Ctrl+P does the same.'
        ),
      },
      {
        keys: 'Ctrl+/',
        detail: t(
          'shortcutCheatsheet',
          'This cheatsheet. ? also opens it when you are not in a text field.'
        ),
      },
      {
        keys: 'Ctrl+Shift+O',
        detail: t(
          'shortcutOverlays',
          'Jump to workspace overlays (chat, browse, Discord guest). In-app only.'
        ),
      },
      {
        keys: 'Ctrl+A',
      ...
      },
      {
        ...
        detail: t(
          'shortcutEsc',
          'Close palette, cheatsheet, legal, admin, or cancelable setup overlays.'
        ),
      },
      {
        keys: 'Enter',
        detail: t('shortcutSend', 'Send the conversation. Shift+Enter inserts a new line.'),
      },
    ];
    if (voice)
      rows.push({
  ...
  function openCheatsheet() {
    ...
    if (box) {
      ...
      for (const row of cheatsheetRows()) {
        ...
        line.append(
          kbd,
          Object.assign(document.createElement('span'), { textContent: row.detail })
        );
        box.append(line);
      }
    }
  ...
  $('#cheatsheetCloseBtn')?.addEventListener('click', () =>
    $('#cheatsheetOverlay')?.classList.add('hidden')
  );
  ...
  $('#cheatsheetOverlay')?.addEventListener('click', e => {
    if (e.target === $('#cheatsheetOverlay')) $('#cheatsheetOverlay').classList.add('hidden');
  });
