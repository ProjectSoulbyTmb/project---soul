...
function populateVoices(){if(!('speechSynthesis' in window))return;const current=$('#voiceSelect').value||(settings?.companion?.voiceName||'');const voices=speechSynthesis.getVoices();$('#voiceSelect').textContent='';const fallback=el('option','','Windows default');fallback.value='';$('#voiceSelect').append(fallback);for(const voice of voices){const option=el('option','',`${voice.name} (${voice.lang})`);option.value=voice.name;$('#voiceSelect').append(option);}$('#voiceSelect').value=[...$('#voiceSelect').options].some(x=>x.value===current)?current:'';}
function applyCompanion(){const c=settings?.companion||{};const gates=state?.policy?.adultStatusConfirmed===true&&state?.policy?.adultSoulEnabled===true&&state?.policy?.currentConsent===true&&state?.policy?.mode==='adult';const avatar=$('#companionAvatar'),mode=c.avatarMode||'3d';avatar.className=`companion-avatar mode-${mode} motion-${c.motion||'gentle'}${gates&&c.adultPresentation?' adult-presentation':''}`;const height=Math.max(0,Math.min(100,Number(c.bodyHeight)||50)),build=Math.max(0,Math.min(100,Number(c.bodyBuild)||50)),curves=Math.max(0,Math.min(100,Number(c.bodyCurves)||50));avatar.style.transform=mode==='2d'?`scaleX(${.82+build*.0036}) scaleY(${.82+height*.0036})`:'';avatar.style.borderRadius=mode==='2d'?`${38+curves*.12}% ${38+curves*.12}% 46% 46%`:'';$('#avatarMode').value=mode;$('#avatarMotion').value=c.motion||'gentle';$('#voiceEnabled').checked=Boolean(c.voiceEnabled);$('#voiceRate').value=c.rate||1;$('#voicePitch').value=c.pitch||1;$('#adultAvatarSettings').classList.toggle('hidden',!gates);$('#adultPresentation').checked=gates&&Boolean(c.adultPresentation);$('#bodyHeight').value=c.bodyHeight??50;$('#bodyBuild').value=c.bodyBuild??50;$('#bodyCurves').value=c.bodyCurves??50;populateVoices();}
function speakSoul(text,force=false){if(!('speechSynthesis' in window)||(!force&&!settings?.companion?.voiceEnabled))return;speechSynthesis.cancel();const utterance=new SpeechSynthesisUtterance(String(text||'').slice(0,8000));const c=settings?.companion||{};utterance.rate=Number(c.rate)||1;utterance.pitch=Number(c.pitch)||1;utterance.voice=speechSynthesis.getVoices().find(v=>v.name===c.voiceName)||null;utterance.onstart=()=>$('#companionAvatar').classList.add('speaking');utterance.onend=utterance.onerror=()=>$('#companionAvatar').classList.remove('speaking');speechSynthesis.speak(utterance);}
...
function renderDashboard(){
  ...
  const items=[
    {label:t('dashFocus','Focus'), value:state.assistant?.preferences?.focusMode||'general', next:t('focusSession','Focus session'), run:()=>{setView('chat');send('Plan a focused session for my current priority.');}},
...
$('#voicePreviewBtn').addEventListener('click',()=>{settings.companion={...(settings.companion||{}),voiceName:$('#voiceSelect').value,rate:Number($('#voiceRate').value),pitch:Number($('#voicePitch').value)};speakSoul('Hello. I am Soul, your Eidovara companion.',true);});
const Recognition=window.SpeechRecognition||window.webkitSpeechRecognition;if(Recognition){$('#voiceInputBtn').classList.remove('hidden');const recognition=new Recognition();recognition.continuous=false;recognition.interimResults=false;recognition.onstart=()=>{$('#voiceSupportStatus').textContent='Listening… microphone audio is used only for this dictation session.';};recognition.onresult=e=>{$('#messageInput').value=[...e.results].map(r=>r[0].transcript).join(' ');autoSize();};recognition.onerror=e=>{$('#voiceSupportStatus').textContent=`Voice input unavailable: ${e.error}. You can continue typing.`;};recognition.onend=()=>{if(!$('#voiceSupportStatus').textContent.startsWith('Voice input unavailable'))$('#voiceSupportStatus').textContent='Dictation complete. Review the text before sending.';};$('#voiceInputBtn').addEventListener('click',()=>recognition.start());}else{$('#voiceSupportStatus').textContent='Installed Windows voices are available for playback. Voice input is not supported by this system build.';}
...
$('#privacyNoticeBtn').addEventListener('click',()=>window.soul.openExternal('https://projectsoulbytmb.github.io/project---soul/privacy.html'));
$('#termsNoticeBtn').addEventListener('click',()=>window.soul.openExternal('https://projectsoulbytmb.github.io/project---soul/terms.html'));
$('#ageNoticeBtn').addEventListener('click',()=>window.soul.openExternal('https://projectsoulbytmb.github.io/project---soul/age.html'));
$('#securityCenterBtn').addEventListener('click',()=>window.soul.openExternal('https://projectsoulbytmb.github.io/project---soul/security.html'));
$('#licensingBtn').addEventListener('click',()=>window.soul.openExternal('https://projectsoulbytmb.github.io/project---soul/licensing.html'));
...
$('#ageGateAcceptBtn').addEventListener('click',async()=>{if(!$('#ageGateTermsCheck').checked)return;settings=await window.soul.acceptAgeGate(true);setAgeGated(false);await refreshBackups().catch(()=>{});renderDashboard();void refreshServiceStatus(true);if(!state.setup?.completed)openSetup(false);else {$('#messageInput').focus();if(settings.updateChannelConfigured)checkUpdates(true);}});
$('#ageGateDeclineBtn').addEventListener('click',()=>window.soul.declineAgeGate());

(async function init(){ try{[state,settings]=await Promise.all([window.soul.snapshot(),window.soul.getSettings()]);$('#providerSelect').value=settings.provider;$('#endpointInput').value=settings.endpoint||'';$('#modelInput').value=settings.model||'';$('#apiKeyInput').placeholder=settings.hasApiKey?'Stored securely — leave blank to keep':'API key';$('#searchApiKeyInput').placeholder=settings.hasSearchApiKey?'Stored securely — leave blank to keep':'Brave Search API key';const theme=settings.theme||{};$('#themeBackground').value=theme.background||'#000000';$('#themePanel').value=theme.panel||'#1c1c1e';$('#themeAccent').value=theme.accent||'#0a84ff';$('#themeTransparency').value=theme.transparency||96;$('#themeTransparencyValue').textContent=`${theme.transparency||96}%`;$('#themeRgb').checked=Boolean(theme.rgbEffects);$('#gamingModeInput').checked=Boolean(theme.gamingMode);if($('#serviceUrlInput'))$('#serviceUrlInput').value=settings.serviceUrl||'';renderAll();setView('chat');window.addEventListener('eidovara:locale',()=>{const mediaBtn=$('#openLocalMediaBtn');if(mediaBtn)mediaBtn.textContent=t('openLocalMedia','Open local media');renderAll();});if(!settings.ageGateAccepted){setAgeGated(true);}else{void refreshServiceStatus(true);await refreshBackups().catch(()=>{});renderDashboard();$('#messageInput').focus();if(!state.setup?.completed)openSetup(false);if(settings.updateChannelConfigured)checkUpdates(true);}}catch(err){document.body.textContent=`Eidovara could not initialize: ${err?.message||err}`;} })();
