...
function runKernelAction(action){
  ...
  else if(action.type==='open-palette') window.eidovaraLayers?.openPalette?.();
  else if(action.type==='open-cheatsheet') window.eidovaraLayers?.openCheatsheet?.();
  ...
  else if(action.type==='run-command' && action.command && typeof window.eidovaraSend==='function') window.eidovaraSend(action.command);
}
...
document.addEventListener('keydown',e=>{
  ...
  if(overlayOpen('#commandPalette') && !$('#paletteResults')){
    const items=filteredPalette($('#paletteInput')?.value);
...
$('#paletteOpenBtn')?.addEventListener('click',()=>{ if(!$('#paletteResults')) openPalette(); });
$('#shortcutBtn')?.addEventListener('click',()=>openShortcutSheet());
$('#cheatsheetOpenBtn')?.addEventListener('click',()=>{ if(!$('#cheatsheetList')) openShortcutSheet(); });
