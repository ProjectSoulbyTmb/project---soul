...
$('#ageGateAcceptBtn').addEventListener('click',async()=>{if(!$('#ageGateTermsCheck').checked)return;settings=await window.soul.acceptAgeGate(true);state=await window.soul.snapshot();setAgeGated(false);await refreshBackups().catch(()=>{});renderDashboard();void refreshServiceStatus(true);if(!state.setup?.completed)openSetup(false);else {$('#messageInput').focus();if(settings.updateChannelConfigured)checkUpdates(true);}});
