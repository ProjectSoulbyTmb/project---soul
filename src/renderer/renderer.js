...
async function send(text, opts={}){
  ...
  try{
    ...
    if(surface!=='companion' && $('#assistThisMessage')?.checked){
      try{
        const assist=await window.soul.assistQuery(text);
        $('#assistThisMessage').checked=false;
        const note=el('div',assist.ok?'research-panel':'error-note');
        note.append(el('div','research-title','Worker helper · not Soul · no transcript sent'));
        ...
        $('#messages').append(el('div','error-note',`Worker helper: ${err?.message||err}`));
      }
    }
