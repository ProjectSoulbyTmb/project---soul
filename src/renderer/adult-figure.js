...
const LIGHTS = {
  studio: { keyDir: [0.45, 0.75, 0.5], keyCol: [1.0, 0.94, 0.88], fillDir: [-0.6, 0.2, 0.3], fillCol: [0.25, 0.32, 0.45], rimDir: [-0.2, 0.1, -1], rimCol: [0.55, 0.62, 0.8], wrap: 0.28 },
  soft: { keyDir: [0.2, 0.9, 0.4], keyCol: [1.0, 0.96, 0.93], fillDir: [-0.4, 0.5, 0.4], fillCol: [0.45, 0.42, 0.4], rimDir: [0, 0.2, -1], rimCol: [0.4, 0.38, 0.36], wrap: 0.55 },
  club: { keyDir: [0.6, 0.4, 0.4], keyCol: [1.0, 0.35, 0.55], fillDir: [-0.5, 0.3, 0.5], fillCol: [0.2, 0.45, 0.9], rimDir: [0, 0.4, -1], rimCol: [0.9, 0.2, 0.7], wrap: 0.2 },
  neon: { keyDir: [0.3, 0.6, 0.5], keyCol: [0.4, 0.9, 1.0], fillDir: [-0.7, 0.2, 0.2], fillCol: [0.9, 0.2, 0.85], rimDir: [0.2, 0.1, -1], rimCol: [0.2, 1.0, 0.7], wrap: 0.22 },
  bedroom: { keyDir: [0.15, 0.55, 0.45], keyCol: [1.0, 0.72, 0.55], fillDir: [-0.35, 0.25, 0.4], fillCol: [0.25, 0.12, 0.18], rimDir: [0.1, 0.3, -1], rimCol: [0.55, 0.28, 0.22], wrap: 0.42 }
};
...
export function attachAdultFigure(canvas) {
  if (!canvas) return { setAvatar() {}, destroy() {}, backend: { webgl: false, ...FIGURE_BACKEND } };
  ...
  let life = { behavior: 'idle-breathe', pace: 'medium', heat: 45, slowMo: false, reducedMotion: false, motion: { breath: 55, sway: 48 } };
  ...
  function drawCanvasFallback() {
    ...
    const hip = layout.hip, waist = layout.waist, bust = layout.bust, shoulder = layout.shoulder;
    body.moveTo(0, 20);
    ...
    tg.addColorStop(0.45, `rgb(${Math.round(skin.r * 255)},${Math.round(skin.g * 255)},${Math.round(skin.b * 255)})`);
