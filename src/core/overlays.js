...
export const OVERLAY_INVENTORY = Object.freeze([
  { kind: 'chat', title: 'Soul chat overlay', mode: 'local', partition: '', injectsGames: false },
  {
    ...
    mode: 'guest',
    partition: 'persist:eidovara-guest',
    injectsGames: false,
  },
  {
    kind: 'discord',
    ...
    mode: 'guest',
    partition: 'persist:eidovara-guest-discord',
    injectsGames: false,
  },
]);
...
export function normalizeOverlayLayout(input, prev = defaultOverlayLayout()) {
  ...
  for (const kind of KINDS) {
    next[kind] = normalizeOverlayBounds(
      kind,
      incoming[kind] !== undefined ? incoming[kind] : prior[kind],
      defaults[kind]
    );
  }
  return next;
  ...
  return [
    {
      ...
      keywords: ['soul', 'companion', 'popout', 'overlay'],
      action: { type: 'open-chat-overlay', label: 'Soul chat overlay' },
    },
    {
      id: 'cmd-overlay-browse',
      kind: 'command',
      title: 'Open browse overlay',
      keywords: ['https', 'guest', 'web', 'overlay'],
      action: { type: 'open-browse-overlay', label: 'Browse overlay' },
    },
    {
      id: 'cmd-overlay-discord',
      kind: 'command',
      title: 'Open Discord overlay',
      keywords: ['discord.com', 'guest', 'invite', 'overlay'],
      action: { type: 'open-discord-overlay', label: 'Discord guest overlay' },
    },
    {
      id: 'cmd-overlay-now-playing',
      kind: 'command',
      title: 'Now playing',
      keywords: ['media', 'eidovara-media', 'player'],
      action: { type: 'open-now-playing', label: 'Now playing' },
    },
    {
      id: 'cmd-always-on-top',
      kind: 'command',
      title: 'Keep Eidovara on top',
      keywords: ['always on top', 'pin window'],
      action: { type: 'set-always-on-top', on: true, label: 'Always on top' },
    },
  ];
}
