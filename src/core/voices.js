...
export function resolveVoice(synth, voiceURI) {
  ...
  return (
    listInstalledVoices(synth).find(voice => voice.voiceURI === wanted || voice.name === wanted) ||
    null
  );
}
...
export function speakText(
  ...
  text,
  settings = {},
  { force = false, Utterance = globalThis.SpeechSynthesisUtterance } = {}
) {
  ...
  if (match && typeof synth.getVoices === 'function') {
    const raw = (synth.getVoices() || []).find(
      item => item.voiceURI === match.voiceURI || item.name === match.name
    );
