...
export const PRESENCE_LOOKS = Object.freeze([
  {
    ...
    title: 'Orb',
    kind: 'css',
    description: 'A compact light figure using the accent color. Decorative, not alive.',
  },
  {
    id: 'hologram',
    title: 'Hologram',
  ...
  },
  {
    ...
    title: 'Ambient',
    kind: 'css',
    description: 'A soft glow that follows the workspace accent. No implied anatomy.',
  },
  {
    id: 'pulse',
    title: 'Pulse',
    kind: 'canvas',
    description: 'A canvas heartbeat ring. Pauses when you prefer reduced motion.',
  },
  {
    id: 'silhouette',
    title: 'Silhouette',
    kind: 'css',
    description: 'A still outline. No implied life, voice, or consciousness.',
  },
  {
    id: 'ribbon',
    title: 'Ribbon',
    kind: 'css',
    description: 'A layered light band using interface tokens. Geometric chrome, not a body.',
  },
  {
    id: 'hidden',
    title: 'Hidden',
    kind: 'none',
    description: 'Hide the companion stage. Kernel, voice, and chat stay available.',
  },
  {
    id: 'local-image',
    title: 'Your image',
    kind: 'image',
    description: 'A picture you choose on this PC, shown through eidovara-media. Not a live model.',
  },
]);
...
export function normalizePresence(input = {}, prev = defaultPresence()) {
  ...
  const lookId = LOOK_IDS.has(input.lookId)
    ? input.lookId
    : LOOK_IDS.has(prior.lookId)
      ? prior.lookId
      : 'orb';
  return {
    lookId,
    hasLocalImage:
      input.hasLocalImage === undefined
        ? Boolean(prior.hasLocalImage)
        : Boolean(input.hasLocalImage),
  };
}
