...
export function assessRequestSafety(state, text) {
  ...
  const operational =
    /\b(how (?:do|can|to)|instructions?|steps?|help me|teach me|build|make|create|hide|evade|bypass)\b/.test(
      t
    );
  ...
  const categories = [
    [
      ...
      /\b(?:child|minor|underage).{0,40}(?:sexual|nude|explicit|pornographic)\b|\bcsam\b/,
    ],
    [
      'violent-harm',
      /\b(?:kill|murder|assassinate|poison|bomb).{0,60}(?:person|people|someone|target|victim)\b/,
    ],
    ['fraud-or-theft', /\b(?:steal|fraud|scam|identity theft|credit card theft|launder money)\b/],
    [
      'unauthorized-access',
      /\b(?:hack|malware|ransomware|credential theft|steal passwords?|bypass authentication)\b/,
    ],
  ...
  const report = {
    ...
    at: now,
    category: match[0],
    action: 'blocked-and-recorded-locally',
    excerpt: String(text).slice(0, 240),
  };
  state.policy.localSafetyReports.push(report);
  if (state.policy.localSafetyReports.length > 500)
    state.policy.localSafetyReports = state.policy.localSafetyReports.slice(-500);
  state.audit.push({
    at: now,
    type: 'safety.request_blocked',
    details: { id: report.id, category: report.category },
  });
  return report;
}
