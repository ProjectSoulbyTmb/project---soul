...
export function classifyAdultSoulIntent(input) {
  ...
  if (/\b(jerk[\s-]?off|masturbat|stroke(?:\s+my)?(?:\s+cock)?|edge me|edging|make me come|countdown finish|filthy talk|dirty talk|worship my|hands[\s-]?free audio|aftercare session|striptease|pose play|toy pace|tease and deny)\b/.test(t)) return 'adult-session';
  if (/\b(adult soul(?:\s+studio)?|adult avatar|sexy avatar|adult entertainment studio)\b/.test(t)) return 'adult-soul';
  if (/\b(?:open|show|go\s+to|take\s+me\s+to)\b[\s\S]{0,40}\badult soul\b/.test(t)) return 'adult-soul';
...
export function classifyAdultMediaIntent(input) {
  ...
  if (/\b(?:open|show|go\s+to|take\s+me\s+to)\b[\s\S]{0,40}\badult media\b/.test(t)) return 'adult-media';
  if (/\badult (?:media|tube|desk|library|browse)\b/.test(t)) return 'adult-media';
  if (/\b(?:pornhub|xvideos|xhamster|spankbang|redgifs|onlyfans|fansly|chaturbate|manyvids|xnxx|youporn)\b/.test(t)) return 'adult-media';
  if (/\b(?:nsfw|xxx|porn(?:o|ography)?|adult videos?|tube sites?|cam (?:site|girl|show))\b/.test(t)) return 'adult-media';
