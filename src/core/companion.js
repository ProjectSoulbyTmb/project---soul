...
const NAV_RULES = [
  { id: 'apps', re: /\b(?:open|show|go\s+to|take\s+me\s+to|launch)\b[\s\S]{0,48}\b(?:apps?(?:\s*&\s*gaming)?|gaming\s+hub)\b/i },
  { id: 'entertainment', re: /\b(?:open|show|go\s+to|take\s+me\s+to)\b[\s\S]{0,40}\bentertainment\b/i },
  { id: 'memory', re: /\b(?:open|show|go\s+to|take\s+me\s+to)\b[\s\S]{0,40}\bmemory\b/i },
  { id: 'identity', re: /\b(?:open|show|go\s+to)\b[\s\S]{0,40}\b(?:identity|consent|adult\s+mode)\b/i },
  ...
  { id: 'setup', re: /\b(?:open|show|configure)\b[\s\S]{0,40}\b(?:assistant\s+setup|soul\s+setup|setup\s+roles)\b/i }
];
...
export function answerCompanion(input, { state } = {}) {
  ...
  if (!text) {
    return {
      ...
      knowledgeId: null,
      usedKnowledge: false,
      reply: 'Ask a short question about this workspace, 18+, legal notices, or a next step. Answers stay on this PC.',
      actions: [
        action('open-view', { view: 'apps', label: 'Apps & Gaming' }),
        action('open-legal', { legal: 'about', label: 'Legal' }),
        action('open-setup', { label: overlay.enabled ? 'Adjust Soul setup' : 'Optional Soul setup' })
      ],
      soul: overlay,
      ...COMPANION_NETWORK
    };
