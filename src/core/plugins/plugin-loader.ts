...
import { PluginManifest, PluginManifestInput, PluginValidationResult, PluginPermission, PluginManifestInput } from './plugin-manifest.js';
...
export class PluginLoader {
  ...
  private async validateManifest(manifest: any): Promise<{ valid: boolean; errors: string[]; warnings: string[]; manifest?: any }> {
    const errors: string[] = [];
    ...
    if (!manifest.entrypoints || (!manifest.entrypoints.main && !manifest.entrypoints.ui && !manifest.entrypoints.commands?.length)) {
      errors.push('At least one entrypoint required');
    }

    if (manifest.permissions) {
      const validPermissions = ['memory', 'filesystem', 'network', 'shell', 'notifications', 'clipboard', 'idle', 'power', 'idleDetection'];
      for (const perm of Object.keys(manifest.permissions)) {
        if (!['memory', 'filesystem', 'network', 'shell', 'notifications', 'clipboard', 'idle', 'power', 'idleDetection'].includes(perm)) {
  ...
  private async loadPluginModule(manifest: any, sandbox: any): Promise<Record<string, any>> {
    ...
    return {
      default: {
        ...
        cleanup: async () => { console.log('Plugin cleaned up'); }
      },
      commands: [],
      hooks: []
    };
