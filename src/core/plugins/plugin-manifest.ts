...
export interface PluginCommandParam {
  name: string;
  type: 'string' | 'number' | 'boolean' | 'select' | 'file' | 'directory' | 'color' | 'date' | 'time' | 'json';
  description: string;
  required?: boolean;
  default?: any;
  options?: { value: string; label: string }[];
...
export type PluginManifestInput = Omit<PluginManifest, 'manifest_version' | 'validated' | 'validatedAt' | 'checksum' | 'signature'>;

export const PLUGIN_MANIFEST_VERSION = 1;
export const PLUGIN_PERMISSION_TYPES = [
  'memory', 'filesystem', 'network', 'shell', 
  'notifications', 'clipboard', 'idle', 'power', 'idle-detection'
] as const;
