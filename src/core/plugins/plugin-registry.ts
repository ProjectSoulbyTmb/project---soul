...
import { PluginManifest, PluginManifestInput, PluginValidationResult, PluginPermission } from './plugin-manifest.js';
...
export interface PluginRegistryEvents {
  ...
  onPluginStateChange: (pluginId: string, state: 'loading' | 'loaded' | 'error' | 'unloaded') => void;
}
...
export function createPluginRegistryWithOptions(options: {
  pluginsDir: string;
  enableHotReload?: boolean;
  maxPlugins?: number;
  autoLoad?: boolean;
