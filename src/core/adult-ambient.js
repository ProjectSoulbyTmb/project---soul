...
export const AMBIENT_HONESTY =
  'Heartbeat, breath, and drone are generated in Chromium Web Audio on this PC. They are not a bundled neural TTS pack and not a porn sound library. Import your own moans through eidovara-media if you want realistic bed audio.';
...
export function ambientLevels(tMs = 0, sounds = {}, feelLevel = 0) {
  ...
  return {
    ...
    breath:
      ambient.breath === false
        ? 0
        : round3((0.42 + 0.58 * (0.5 + 0.5 * Math.sin(t * Math.PI * 0.26))) * master * 0.55),
