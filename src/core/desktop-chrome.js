...
export const SLEEP_PRESETS_MS = Object.freeze({
  off: 0,
  15: 15 * 60_000,
  30: 30 * 60_000,
  60: 60 * 60_000,
...
export function normalizeDesktopChrome(input = {}, prior = defaultDesktopChrome()) {
  ...
  return {
    ...
    sleepUntil:
      typeof base.sleepUntil === 'string' && base.sleepUntil
        ? String(base.sleepUntil).slice(0, 40)
        : null,
  };
}
...
export function pushNotice(list, notice, { at } = {}) {
  ...
  return [next, ...normalizeNotices(list).filter(item => item.id !== next.id)].slice(
    0,
    CHROME_NOTICE_LIMIT
  );
}
...
export function evaluateArithmetic(query) {
  ...
  try {
    ...
    return {
      kind: 'calc',
      title: `${text.replace(/\s+/g, ' ').trim()} = ${round4(result)}`,
