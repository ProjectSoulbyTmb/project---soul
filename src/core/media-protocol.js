...
export function playbackUrlFromCommons(info = {}, kind = 'image') {
  const original = String(info.url || '');
  const thumb = String(info.thumburl || info.thumbUrl || '');
  if (kind === 'image') return thumb || original;
  return original || thumb;
}

export function downscaleForbidden(params = {}) {
  const width = Number(params.iiurlwidth || params.width || 0);
  const kind = String(params.kind || params.filetype || '');
