...
export const FANTASY_FRAMEWORKS = Object.freeze([
  ...
  {
    id: 'high-elf',
    title: 'High elf (adult)',
    hint: 'Pointed ears, longer limbs. Locked 21+ — not a childlike elf.',
  },
  {
    id: 'succubus',
    title: 'Succubus aesthetic',
    hint: 'Horn nubs + extra sheen. Costume, not a claimed entity.',
  },
  { id: 'vampire', title: 'Vampire look', hint: 'Paler skin, cooler rim. Adult costume.' },
  {
    id: 'kitsune',
    title: 'Kitsune (adult)',
    hint: 'Tail mass at the hips. Folklore costume, adult only.',
  },
  ...
  {
    id: 'fae-court',
    title: 'Fae court (adult)',
    hint: 'Adult court fashion only — not a child fairy.',
  },
]);
...
export const ATMOSPHERE_SCENES = Object.freeze([
  {
    id: 'bedroom',
    title: 'Bedroom',
    hint: 'Warm lamps, sheets implied',
    lighting: 'bedroom',
    overlay: 'lamps',
  },
  ...
  },
  {
    id: 'cinema-dark',
    title: 'Cinema dark',
    hint: 'Letterbox, projector spill',
    lighting: 'studio',
    overlay: 'grain',
  },
  {
  ...
  },
  {
    id: 'dungeon-soft',
    title: 'Soft dungeon',
    hint: 'Consensual club lighting. No breath-play.',
    lighting: 'club',
    overlay: 'velvet',
  },
  {
  ...
  },
  {
    id: 'infinity-pool',
    title: 'Night pool',
    hint: 'Cyan bounce, adult swim',
    lighting: 'neon',
    overlay: 'steam',
  },
  {
    id: 'gym-after',
    title: 'Gym after hours',
    hint: 'Adult athletic space, 21+',
    lighting: 'studio',
    overlay: 'led',
  },
]);
...
export const HUD_SHORTCUTS = Object.freeze([
  {
    key: '`',
    title: 'Boss key',
    hint: 'Blank the stage instantly. Toggle again to restore. Does not hide the process from Task Manager.',
  },
...
export const SHOW_HONESTY =
  'Adult Soul entertainment is a local theater around a first-party mesh: rooms, wardrobe tints, adult fantasy costumes, show packs, a HUD, and a boss key. It is not a livestream of a person, not VRM, and not consciousness. Backtick blanks the stage. Red / safeword stops the session. Revoke Adult Mode anytime.';
...
export function clothingTint(wear, t) {
  ...
  if (id === 'sheer') {
    mix = chest || hips ? 0.18 : 0.06;
    gloss = 0.2;
  } else if (id === 'lingerie' || id === 'garter' || id === 'bikini') {
    mix = hips || chest ? 0.35 : 0;
    gloss = 0.12;
  } else if (id === 'harness' || id === 'choker-set') {
    mix = chest || waist || neck ? 0.45 : 0;
    gloss = 0.08;
  } else if (id === 'stockings') {
    mix = legs ? 0.4 : 0;
    gloss = 0.15;
  } else if (id === 'corset') {
    mix = waist ? 0.55 : chest ? 0.2 : 0;
    gloss = 0.1;
  } else if (id === 'latex-look' || id === 'wet-look' || id === 'bodysuit') {
    mix = chest || hips || waist ? 0.5 : 0.12;
    gloss = 0.65;
  } else if (id === 'jock' || id === 'boxers') {
    mix = hips && t > 0.44 && t < 0.58 ? 0.5 : 0;
    gloss = 0.1;
  } else if (id === 'shirt-only' || id === 'mesh-top') {
    mix = chest || (t > 0.12 && t < 0.4) ? 0.38 : 0;
    gloss = 0.05;
  } else if (id === 'wrap-skirt') {
    mix = hips || (t > 0.4 && t < 0.7) ? 0.42 : 0;
    gloss = 0.06;
  } else if (id === 'jewelry-only') {
    mix = neck || (hips && Math.abs(t - 0.5) < 0.03) ? 0.55 : 0;
    gloss = 0.7;
  } else if (
    id === 'towel' ||
    id === 'robe' ||
...
export function frameworkSkin(framework, skin) {
  ...
  if (id === 'vampire')
    return {
      ...skin,
      tone: '#d7c4c0',
      tan: Math.min(skin.tan, 12),
      blush: Math.max(skin.blush, 28),
    };
  if (id === 'high-elf' || id === 'fae-court')
    return { ...skin, tone: '#e2c6a8', sheen: Math.max(skin.sheen, 40) };
  if (id === 'orc') return { ...skin, tone: '#6f8a4a', tan: 20 };
  if (id === 'succubus' || id === 'dragonkin')
    return { ...skin, sheen: Math.max(skin.sheen, 55), blush: Math.max(skin.blush, 40) };
...
export function sexyStylePatch(style) {
  ...
  if (id === 'glamour')
    return {
      makeup: { lids: 70, liner: 62, blush: 58, lips: 72 },
      skin: { sheen: 55 },
      motion: { sway: 62 },
    };
  if (id === 'brat') return { motion: { sway: 70, idle: 65 }, makeup: { lips: 68, liner: 50 } };
  if (id === 'jock')
    return { figure: { shoulders: 72, chest: 70, waist: 42, bust: 40 }, presentationWear: 'jock' };
  if (id === 'succubus-glam')
    return {
      skin: { sheen: 72, blush: 60 },
      explicit: { nipples: 80, assFocus: 70 },
      presentationWear: 'sheer',
    };
  if (id === 'femme-fatale')
    return { makeup: { liner: 74, lips: 70, lids: 60 }, motion: { eyeContact: 88, sway: 40 } };
  ...
  if (id === 'athletic-cut')
    return { figure: { belly: 12, chest: 64, waist: 36, thighs: 62 }, skin: { tan: 40 } };
  if (id === 'exhibitionist')
    return { presentationWear: 'bare', motion: { eyeContact: 90, sway: 58 }, skin: { sheen: 48 } };
  if (id === 'voyeur-cam')
    return { motion: { eyeContact: 30, sway: 55 }, renderLighting: 'studio' };
  if (id === 'goth-glam')
    return { makeup: { lids: 82, liner: 80, lips: 70, blush: 30 }, skin: { sheen: 42 } };
  ...
  if (id === 'gym-after')
    return {
      figure: { shoulders: 68, chest: 66, waist: 38 },
      presentationWear: 'jock',
      skin: { tan: 44 },
    };
  return {};
}
...
export function touchZoneAt(t) {
  ...
  return (
    TOUCH_ZONES.find(zone => y >= zone.y0 && y < zone.y1) || TOUCH_ZONES[TOUCH_ZONES.length - 1]
  );
}
...
export function theaterClasses(stage = {}, session = {}) {
  ...
  if (stage.afterglow === true || session.kind === 'afterglow-hold' || session.kind === 'aftercare')
    classes.push('is-afterglow');
