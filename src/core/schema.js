...
export function defaultProfile(profileId = 'default') {
  ...
  return {
    ...
    continuity: {
      ...
      lastActiveAt: now,
      reflectionState: {
        growthInsightCount: 0,
        latestReflection: 'Soul begins from humility, care, and user-directed growth.',
        activeTheme: 'baseline-continuity',
      },
      selfModel: {
        name: 'Soul',
        architecture:
          'persistent software continuity and self-model; not a claim of phenomenal consciousness',
    ...
    relationship: {
      style: 'balanced',
      temporaryInitiative: false,
      initiativeReason: null,
      establishedPreference: null,
      trust: 0.5,
      comfort: 0.5,
      auditTrail: [],
    },
    assistant: {
      ...
      identityDescription: 'persistent simulated continuity',
      ethicalFramework: [
        'lawfulness',
        'human safety',
        'consent',
    ...
    setup: {
      ...
      categories: [],
      customNeeds: '',
      stream: { enabled: false, obsWebSocketUrl: 'ws://127.0.0.1:4455', goals: '' },
    },
    entertainment: {
      favorites: [],
      history: [],
    ...
    feedback: [],
    conversations: [
      { id: 'main', title: 'Conversation', createdAt: now, updatedAt: now, messages: [] },
    ],
...
export function migrateProfile(input, profileId = 'default') {
  ...
  const merged = {
    ...
    continuity: {
      ...base.continuity,
      ...(input.continuity || {}),
      selfModel: { ...base.continuity.selfModel, ...(input.continuity?.selfModel || {}) },
      reflectionState: {
        ...base.continuity.reflectionState,
        ...(input.continuity?.reflectionState || {}),
      },
    },
    ...
    assistant: {
      ...base.assistant,
      ...(input.assistant || {}),
      preferences: { ...base.assistant.preferences, ...(input.assistant?.preferences || {}) },
      capabilities: { ...base.assistant.capabilities, ...(input.assistant?.capabilities || {}) },
    },
    policy: { ...base.policy, ...(input.policy || {}) },
    setup: {
      ...base.setup,
      ...(input.setup || {}),
      stream: { ...base.setup.stream, ...(input.setup?.stream || {}) },
    },
  ...
  merged.conversations = merged.conversations
    ...
    .map((c, index) => ({
      ...
      title: String(c.title || `Conversation ${index + 1}`).slice(0, 120),
      createdAt: c.createdAt || base.createdAt,
      updatedAt: c.updatedAt || c.createdAt || base.updatedAt,
  ...
  if (
    !merged.entertainment.taste ||
    typeof merged.entertainment.taste !== 'object' ||
    Array.isArray(merged.entertainment.taste)
  )
    merged.entertainment.taste = {};
  if (
    !merged.entertainment.adult ||
    typeof merged.entertainment.adult !== 'object' ||
    Array.isArray(merged.entertainment.adult)
  ) {
    merged.entertainment.adult = base.entertainment.adult;
  }
  if (
    !merged.adultSoul ||
    typeof merged.adultSoul !== 'object' ||
    Array.isArray(merged.adultSoul)
  ) {
    merged.adultSoul = base.adultSoul;
  }
  if (!Array.isArray(merged.relationship.auditTrail)) merged.relationship.auditTrail = [];
  if (!['user-led', 'balanced', 'proactive'].includes(merged.assistant.autonomy))
    merged.assistant.autonomy = 'balanced';
  if (!Array.isArray(merged.assistant.ethicalFramework))
    merged.assistant.ethicalFramework = [...base.assistant.ethicalFramework];
  if (!['concise', 'balanced', 'detailed'].includes(merged.assistant.preferences.responseLength))
    merged.assistant.preferences.responseLength = 'balanced';
  if (
    !['natural', 'direct', 'warm', 'professional', 'playful'].includes(
      merged.assistant.preferences.tone
    )
  )
    merged.assistant.preferences.tone = 'natural';
  if (!['en', 'es', 'fr', 'de'].includes(merged.assistant.preferences.language))
    merged.assistant.preferences.language = 'en';
  if (
    !merged.activeConversationId ||
    !merged.conversations.some(c => c.id === merged.activeConversationId)
  )
