...
export const SETTINGS_LABELS = Object.freeze([
  {
    ...
    keywords: ['locale', 'español', 'français', 'deutsch'],
    view: 'settings',
    panel: 'assistantBehaviorForm',
  },
  {
    id: 'set-provider',
    title: 'Conversation engine provider',
    keywords: ['offline', 'ollama', 'endpoint'],
    view: 'settings',
  },
  {
    id: 'set-service',
    title: 'Eidovara service URL',
    keywords: ['worker', 'connect', 'health'],
    action: { type: 'open-service' },
  },
  {
    id: 'set-assist',
    title: 'Optional Worker helper /v1/assist',
    keywords: ['soul-online', 'opt-in', 'assist'],
    action: { type: 'open-service' },
  },
  {
    id: 'set-voice',
    title: 'OS voice and mute',
    keywords: ['speech', 'speechSynthesis'],
    view: 'settings',
  ...
  },
  {
    id: 'set-presence',
    title: 'Presence look',
    keywords: ['orb', 'hologram', 'companion'],
    view: 'settings',
    panel: 'kernelCustomizeForm',
  },
  {
  ...
  },
  {
    ...
    title: 'Accessibility or interaction needs',
    keywords: ['reduced motion', 'keyboard-first', 'screen reader'],
    view: 'settings',
    panel: 'assistantBehaviorForm',
  },
  {
  ...
  },
  {
    ...
    keywords: ['github', 'sha-256'],
    action: { type: 'open-updates' },
  },
]);
...
export const CHEATSHEET_ENTRIES = Object.freeze([
  {
    ...
    id: 'palette',
    summary:
      'Command palette — jump to views, intents, settings, legal, modules, overlays, and linked apps.',
  },
  {
    keys: 'Ctrl+Shift+O',
    id: 'overlays',
    summary:
      'Jump to the Play desk overlay menu (chat, browse, Discord guest). In-app only — does not fire inside other games.',
  },
  {
    keys: 'Ctrl+/',
    also: '?',
    id: 'cheatsheet',
    summary: 'Keyboard cheatsheet. ? is ignored while a text field is focused.',
  },
  {
    keys: 'Ctrl+A',
    id: 'admin',
    summary:
      'Private administrator panel. Does not steal select-all from input, textarea, or select fields.',
  },
  {
    keys: 'Esc',
    id: 'escape',
    summary: 'Close palette, cheatsheet, legal, admin, or cancelable setup overlays.',
  },
...
export function normalizeWorkspaceLayers(input, prev = defaultWorkspaceLayers()) {
  ...
  const widgetsIn =
    incoming.widgets && typeof incoming.widgets === 'object'
      ? incoming.widgets
      : prior.widgets || {};
  ...
  const focusIn =
    incoming.focus && typeof incoming.focus === 'object' ? incoming.focus : prior.focus || {};
  const durationMs = clampDurationMs(focusIn.durationMs, base.focus.durationMs);
  const scratchIn =
    incoming.scratchpad && typeof incoming.scratchpad === 'object'
      ? incoming.scratchpad
      : prior.scratchpad || {};
  return {
    ...
    favorites: uniqueStrings(
      incoming.favorites !== undefined ? incoming.favorites : prior.favorites,
      24,
      80
    ),
  };
...
export function reorderWidgets(state, order) {
  const workspace = ensureWorkspace(state);
  workspace.widgets = normalizeWorkspaceLayers({
    ...workspace,
    widgets: { ...workspace.widgets, order },
  }).widgets;
  return workspace;
}
...
export function isFocusStartCommand(input) {
  ...
  return (
    /\b(start|begin|kick\s*off)\b[\s\S]{0,40}\bfocus\b/.test(t) ||
...
export function isScratchCaptureCommand(input) {
  const t = String(input || '').toLowerCase();
  return (
    /^(note:|scratch:|capture\s+(this\s+)?(scratch(pad)?|note)|save\s+(this\s+)?(scratch(pad)?|note)\s+to\s+memory)\b/.test(
      t
...
export function scratchBodyFromInput(input) {
  return String(input || '')
    .replace(/^(note:|scratch:|remember this scratch:?)\s*/i, '')
    .trim()
    .slice(0, 4000);
}
...
export function startFocusSession(state, { minutes, label, at, now } = {}) {
  ...
  state.audit.push({
    at: when,
    type: 'workspace.focus_start',
    details: { minutes: mins, quiet: true, killsOtherProcesses: false },
  });
  return workspace.focus;
}
...
export function toggleFavorite(state, id) {
  ...
  if (workspace.favorites.includes(key))
    workspace.favorites = workspace.favorites.filter(item => item !== key);
  else workspace.favorites = [key, ...workspace.favorites].slice(0, 24);
  return workspace.favorites;
}

function haystack(item) {
  return [item.title, item.summary, item.label, ...(item.keywords || [])]
    .filter(Boolean)
    .join(' ')
    .toLowerCase();
}
  ...
  const views = [
    {
      ...
      kind: 'view',
      title: 'Dashboard',
      keywords: ['home', 'workspace', 'tiles'],
      action: action('open-view', { view: 'dashboard', label: 'Open Dashboard' }),
    },
    {
      id: 'view-chat',
      kind: 'view',
      title: 'Conversation',
      keywords: ['chat', 'talk', 'composer'],
      action: action('open-view', { view: 'chat', label: 'Open conversation' }),
    },
    {
      id: 'view-apps',
      kind: 'view',
      title: 'Apps & Gaming',
      keywords: ['launch', 'windows', 'start menu'],
      action: action('open-view', { view: 'apps', label: 'Open Apps & Gaming' }),
    },
    {
      id: 'view-entertainment',
      kind: 'view',
      title: 'Entertainment',
      keywords: ['media', 'music', 'mix'],
      action: action('open-view', { view: 'entertainment', label: 'Open Entertainment' }),
    },
    {
      id: 'view-memory',
      kind: 'view',
    ...
    },
    {
      ...
      keywords: ['adult mode', 'self-model'],
      action: action('open-view', { view: 'identity', label: 'Identity & consent' }),
    },
    {
      id: 'view-settings',
      kind: 'view',
      title: 'Settings',
      keywords: ['engine', 'provider', 'backups'],
      action: action('open-view', { view: 'settings', label: 'Open Settings' }),
    },
  ];
  const commands = [
    {
      ...
      keywords: ['timer', 'quiet', 'pomodoro'],
      action: action('start-focus', { minutes: 25, label: 'Focus session' }),
    },
    {
      id: 'cmd-focus-stop',
      kind: 'command',
      ...
      keywords: ['end timer'],
      action: action('stop-focus'),
    },
    {
      id: 'cmd-scratch-capture',
      kind: 'command',
      title: 'Capture scratchpad to memory',
      keywords: ['note', 'remember', 'local'],
      action: action('capture-scratch'),
    },
    {
      id: 'cmd-cheatsheet',
      kind: 'command',
      title: 'Keyboard cheatsheet',
      keywords: ['shortcuts', 'hotkeys', 'help', 'ctrl+/'],
      action: action('open-cheatsheet'),
    },
    {
      id: 'cmd-setup',
      kind: 'command',
      ...
      keywords: ['roles'],
      action: action('open-setup', { label: 'Assistant setup' }),
    },
    {
      id: 'cmd-diagnostics',
      kind: 'command',
      ...
      keywords: ['health', 'gpu'],
      action: action('open-diagnostics', { label: 'Show diagnostics' }),
    },
    {
      id: 'cmd-service',
      kind: 'command',
      title: 'Eidovara service settings',
      keywords: ['worker', 'connect'],
      action: action('open-service', { label: 'Service settings' }),
    },
    {
      id: 'cmd-legal-about',
      kind: 'legal',
      title: 'About & legal',
      keywords: ['unsigned', 'source-available'],
      action: action('open-legal', { legal: 'about', label: 'About & legal' }),
    },
    {
      id: 'cmd-legal-terms',
      kind: 'legal',
      title: 'Terms of use',
      keywords: ['legal'],
      action: action('open-legal', { legal: 'terms', label: 'Terms' }),
    },
    {
      id: 'cmd-legal-privacy',
      kind: 'legal',
      title: 'Privacy notice',
      keywords: ['telemetry', 'local-first'],
      action: action('open-legal', { legal: 'privacy', label: 'Privacy' }),
    },
    {
      id: 'cmd-legal-age',
      kind: 'legal',
      ...
      action: action('open-legal', { legal: 'age', label: 'Age 18+' }),
    },
    ...overlayPaletteItems(),
  ];
  const moduleItems = (Array.isArray(modules) ? modules : [])
    .filter(mod => enabled(mod.id))
    .map(mod => ({
      id: `mod-${mod.id}`,
      kind: 'module',
      title: mod.title,
...
export function searchWorkspace(
  ...
  return filterItems(
    query,
    [...palette, ...settingItems, ...knowledgeItems, ...memoryItems, ...appItems],
    24
  );
}
...
export function cheatsheetEntries({ voiceInput = false } = {}) {
  ...
  if (voiceInput) {
    rows.push({
      ...
      id: 'dictate',
      summary:
        'Hold the dictation control to talk; release to stop. Uses OS speech recognition. Eidovara does not ship a neural TTS engine.',
    });
  }
  return rows;
}
