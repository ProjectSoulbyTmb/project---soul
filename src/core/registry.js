...
export function normalizeCustomAction(input = {}, fallbackId) {
  ...
  const id = String(input.id || fallbackId || `act_${Date.now().toString(36)}`)
...
export function normalizeRegistry(input = {}, prev = defaultRegistry()) {
  ...
  const priorEnabled =
    prior.moduleEnabled && typeof prior.moduleEnabled === 'object' ? prior.moduleEnabled : {};
  const nextEnabled =
    incoming.moduleEnabled && typeof incoming.moduleEnabled === 'object'
      ? incoming.moduleEnabled
      : {};
  const moduleEnabled = { ...base.moduleEnabled };
  for (const mod of builtinModules) {
    if (Object.prototype.hasOwnProperty.call(nextEnabled, mod.id))
      moduleEnabled[mod.id] = nextEnabled[mod.id] !== false;
    else if (Object.prototype.hasOwnProperty.call(priorEnabled, mod.id))
      moduleEnabled[mod.id] = priorEnabled[mod.id] !== false;
  }
  const rawActions = Array.isArray(incoming.customActions)
    ? incoming.customActions
    : Array.isArray(prior.customActions)
      ? prior.customActions
      : [];
  const customActions = rawActions
    .map((item, index) => normalizeCustomAction(item, `act_${index}`))
    .filter(Boolean)
    .slice(0, 24);
  const phrasingIn =
    incoming.phrasing && typeof incoming.phrasing === 'object'
      ? incoming.phrasing
      : prior.phrasing || {};
...
export function createRuntimeRegistry(catalog = builtinModules) {
  ...
  return {
    ...
    enabled(id, persisted) {
      ...
      if (map && typeof map === 'object' && Object.prototype.hasOwnProperty.call(map, id))
        return map[id] !== false;
