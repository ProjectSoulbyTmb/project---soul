...
export function compareVersions(a, b) {
  ...
  const fallbackLeft = parts(a),
    fallbackRight = parts(b);
...
export function requireUpdateIntegrity(info) {
  if (!info || typeof info !== 'object')
    throw new Error('Update metadata is missing. Eidovara refused to install.');
  ...
  if (!isSha512(sha512) && !isSha256(sha256)) {
    throw new Error(
      'Update metadata is missing a SHA-512 or SHA-256 checksum. Eidovara refused to install.'
    );
  }
...
export function parseLatestYml(text) {
  ...
  const fileSha =
    (raw.match(/^\s+-?\s*sha512:\s*['"]?([A-Za-z0-9+/=]+)/m) ||
      raw.match(/^\s+sha512:\s*['"]?([A-Za-z0-9+/=]+)/m) ||
      [])[1] || '';
  const fileUrl =
    (raw.match(/^\s+-?\s*url:\s*['"]?([^\s'"]+)/m) ||
      raw.match(/^\s+url:\s*['"]?([^\s'"]+)/m) ||
      [])[1] || '';
...
export async function checkForUpdate({ manifestUrl, currentVersion }) {
  ...
  const res = await response(manifestUrl, {
    headers: { Accept: 'application/json', 'User-Agent': `Eidovara/${currentVersion}` },
  });
...
export async function downloadUpdate(update, directory, maxBytes = 400 * 1024 * 1024) {
  ...
  const res = await response(update.downloadUrl, {
