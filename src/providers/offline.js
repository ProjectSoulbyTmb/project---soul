...
function pack(locale, mode, opener, bullets, closer, state) {
  ...
  const disclaimer = setupOn ? {
    ...
    en: 'Optional Soul setup is off. This is the Eidovara workspace kernel — software on this device, not a person or a claim of consciousness.',
    es: 'La configuración opcional de Soul está apagada. Este es el núcleo del espacio Eidovara: software en este dispositivo, no una persona ni una afirmación de consciencia.',
    fr: 'La configuration Soul facultative est désactivée. Ceci est le noyau d’espace Eidovara — un logiciel sur cet appareil, pas une personne ni une preuve de conscience.',
    de: 'Optionales Soul-Setup ist aus. Das ist der Eidovara-Arbeitsbereichs-Kernel — Software auf diesem Gerät, keine Person und kein Bewusstseinsanspruch.'
  };
...
export function composeOfflineReply({ input, state, webResearch, view, intent: routedIntent } = {}) {
  const locale = localeOf(state);
  ...
  const boxed = (opener, bullets, closer) => pack(locale, mode, opener, bullets, closer, state);

  if (webResearch) return researchReply(webResearch, locale);

  if (intent === 'identity') {
    return boxed( toneLead(locale, tone,
      setupOn
  ...
  if (intent === 'focus') {
    ...
    return boxed( { en: `Let’s shape a focused session around your ${lens} priority.`, es: `Armemos una sesión concentrada en tu prioridad de ${lens}.`, fr: `Construisons une session concentrée autour de votre priorité ${lens}.`, de: `Lassen Sie uns eine fokussierte Sitzung um Ihre ${lens}-Priorität bauen.` }[locale], [
  ...
  if (intent === 'reassure') {
    return boxed( { en: 'We can slow this down. There is no need to perform for me.', es: 'Podemos ir más despacio. No hace falta actuar para mí.', fr: 'Nous pouvons ralentir. Pas besoin de performer pour moi.', de: 'Wir können langsamer werden. Sie müssen für mich nicht auftreten.' }[locale], [
  ...
  if (intent === 'entertainment') {
    return boxed({ en: 'Entertainment is local taste, a queue helper, and lawful Spotify/YouTube HTTPS searches — plus a local file picker through eidovara-media.', es: 'Entretenimiento es gusto local, cola, y búsquedas HTTPS lícitas de Spotify/YouTube, más un selector de archivos locales via eidovara-media.', fr: 'Divertissement : goût local, file, recherches HTTPS Spotify/YouTube, et un sélecteur de fichier local via eidovara-media.', de: 'Unterhaltung ist lokaler Geschmack, Warteschlange, rechtmäßige Spotify/YouTube-HTTPS-Suchen und ein lokaler Dateiauswahl über eidovara-media.' }[locale], [
      ...
      { en: 'Mood mix, favorites, watch, OST helpers, and Surprise me stay on this PC. No stream ripping.', es: 'Mezcla de ánimo, favoritos, ver, bandas y Sorpréndeme se quedan en este PC. Sin rip de streams.', fr: 'Mix d’humeur, favoris, watch, OST et Surprise restent sur cet appareil. Pas d’extraction de flux.', de: 'Stimmungsmix, Favoriten, Watch, OST und Überrasche mich bleiben auf diesem PC. Kein Stream-Ripping.' }[locale]
  ...
  if (intent === 'backups') {
    return boxed({ en: 'Backups are portable local snapshots. Encrypted when Windows protection is available. Restore replaces the current profile.', es: 'Las copias son instantáneas locales. Cifradas si Windows lo permite. Restaurar reemplaza el perfil actual.', fr: 'Les sauvegardes sont des instantanés locaux. Chiffrées si Windows le permet. Restaurer remplace le profil actuel.', de: 'Sicherungen sind lokale Momentaufnahmen. Verschlüsselt, wenn Windows-Schutz da ist. Wiederherstellen ersetzt das aktuelle Profil.' }[locale], [
  ...
  if (intent === 'setup') {
    return boxed(setupOn
      ...
      roles,
      { en: 'OBS websocket control is not in this release. Stream helper stores local planning notes only.', es: 'El control OBS websocket no está en esta versión. La ayuda de streaming solo guarda notas locales.', fr: 'Le contrôle OBS websocket n’est pas dans cette version. L’aide streaming stocke seulement des notes locales.', de: 'OBS-Websocket-Steuerung ist nicht in dieser Version. Stream-Hilfe speichert nur lokale Planungsnotizen.' }[locale]
  ...
  if (intent === 'forget') {
    return boxed({ en: 'Durable notes can be forgotten. Prefix a line with “forget:” or use Forget on a Memory card.', es: 'Las notas duraderas se pueden olvidar. Prefija con “forget:” o usa Olvidar en la tarjeta.', fr: 'Les notes durables peuvent être oubliées. Préfixez « forget: » ou Forget sur la carte.', de: 'Dauerhafte Notizen können vergessen werden. „forget:“ voranstellen oder Forget auf der Karte.' }[locale], [
  ...
  if (intent === 'here') {
    const hereMap = {
      apps: { en: 'Here you can discover Start Menu shortcuts, confirm-launch a trusted app, or turn on low-overhead gaming mode. No injection.', es: 'Aquí puedes descubrir accesos del menú Inicio, lanzar con confirmación, o activar el modo de bajo consumo. Sin inyección.', fr: 'Ici : raccourcis du menu Démarrer, lancement confirmé, mode allégé. Pas d’injection.', de: 'Hier: Startmenü finden, mit Bestätigung starten, Low-Overhead. Keine Injektion.' },
      ...
      dashboard: { en: 'Here is the workspace home: focus, study, create, gaming checklists, memory review, and this companion.', es: 'Esta es la casa del espacio: enfoque, estudio, crear, listas de juego, memoria y este compañero.', fr: 'Voici l’accueil : focus, étude, création, checklists jeu, mémoire et ce compagnon.', de: 'Das ist die Startseite: Fokus, Lernen, Gestalten, Gaming-Checklisten, Speicher und dieser Begleiter.' }
    };
    const current = ['apps', 'entertainment', 'memory', 'identity', 'settings', 'chat'].includes(view) ? view : 'dashboard';
    return boxed(hereMap[current][locale] || hereMap[current].en, [
  ...
  if (intent === 'help' || intent === 'identity-panel') {
    return boxed( { en: 'I can steer this workspace: dashboard, conversation, apps, entertainment, memory, identity, settings, help/legal, diagnostics, and research after an explicit internet request.', es: 'Puedo orientar este espacio: panel, conversación, apps, entretenimiento, memoria, identidad, ajustes, legal, diagnóstico e investigación tras una petición explícita de internet.', fr: 'Je peux orienter cet espace : tableau, conversation, apps, divertissement, mémoire, identité, paramètres, légal, diagnostics, et recherche après une demande internet explicite.', de: 'Ich steuere diesen Arbeitsbereich: Übersicht, Unterhaltung, Apps, Medien, Speicher, Identität, Einstellungen, Rechtliches, Diagnose und Recherche nach expliziter Internetanfrage.' }[locale], [
      setupOn
        ...
        : { en: 'Optional Soul setup is off. This kernel is software. Assist, if you later opt in, is still not Soul.', es: 'Soul opcional está apagado. Este núcleo es software. Assist, si más tarde lo activas, sigue sin ser Soul.', fr: 'Soul facultatif est désactivé. Ce noyau est un logiciel. Assist, même après opt-in, n’est pas Soul.', de: 'Optionales Soul ist aus. Dieser Kernel ist Software. Assist nach späterem Opt-in ist trotzdem nicht Soul.' }[locale]
    ], { en: 'Ask a workspace next step, or use the chips. Ctrl+/ focuses the companion.', es: 'Pide un siguiente paso, o usa los chips. Ctrl+/ enfoca el compañero.', fr: 'Demandez une prochaine étape, ou utilisez les puces. Ctrl+/ focus le compagnon.', de: 'Fragen Sie einen nächsten Schritt oder nutzen Sie die Chips. Strg+/ fokussiert den Begleiter.' }[locale]);
  }
