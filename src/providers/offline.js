...
export function detectOfflineIntent(input) {
  ...
  if (/\b(gaming or streaming|streaming setup|gaming setup|prepare my gaming|obs)\b/.test(t)) return 'gaming';
  ...
  if (/\b(search the internet|current information i need|web research)\b/.test(t)) return 'research';
  if (/\b(fits my current mood|mood mix|mood and explain)\b/.test(t)) return 'mood';
  if (/\b(similar to my favorite|favorite music)\b/.test(t)) return 'favorites';
  if (/\b(video worth watching|watch something)\b/.test(t)) return 'watch';
  if (/\bgaming soundtrack\b/.test(t)) return 'gaming-ost';
  if (/\b(study soundtrack|calm study)\b/.test(t)) return 'study-ost';
...
function toneLead(locale, tone, warm, direct) {
  ...
  if (tone === 'warm' || tone === 'playful') return warm;
  return locale === 'en' ? warm : warm;
}

function roleHint(state, locale) {
  const roles = rolesOf(state);
  if (!roles.length) {
    return { en: 'Setup roles are not configured yet — open Assistant setup when you want gaming, study, accessibility, or streaming checklists.', es: 'Todavía no hay roles de configuración. Abre la configuración del asistente para juego, estudio, accesibilidad o streaming.', fr: 'Aucun rôle n’est encore configuré. Ouvrez la configuration de l’assistant pour le jeu, les études, l’accessibilité ou le streaming.', de: 'Es sind noch keine Rollen gesetzt. Öffnen Sie die Assistenten-Einrichtung für Gaming, Lernen, Barrierefreiheit oder Streaming.' }[locale];
  }
  const labels = { 'gaming-editing': { en: 'gaming & editing', es: 'juegos y edición', fr: 'jeu et montage', de: 'Gaming und Schnitt' }, 'stream-helper': { en: 'stream helper', es: 'ayuda para streaming', fr: 'aide au streaming', de: 'Stream-Hilfe' }, studying: { en: 'studying', es: 'estudio', fr: 'études', de: 'Lernen' }, personal: { en: 'personal', es: 'uso personal', fr: 'usage personnel', de: 'persönlich' }, creative: { en: 'creative', es: 'creativo', fr: 'création', de: 'kreativ' }, 'work-productivity': { en: 'work', es: 'trabajo', fr: 'productivité', de: 'Arbeit' }, accessibility: { en: 'accessibility', es: 'accesibilidad', fr: 'accessibilité', de: 'Barrierefreiheit' } };
...
function accessHint(state, locale) {
  ...
  return { en: `Accessibility is in scope.${extra} I will keep pacing readable, avoid motion pressure, and prefer keyboard-clear next steps.`, es: `La accesibilidad está en el alcance.${extra} Mantendré un ritmo legible y pasos claros.`, fr: `L’accessibilité est prise en compte.${extra} Je garderai un rythme lisible et des étapes claires.`, de: `Barrierefreiheit gilt hier.${extra} Ich bleibe beim Tempo lesbar und bei klaren Schritten.` }[locale];
}
...
function researchReply(webResearch, locale) {
  ...
  const media = webResearch.media?.length ? ({ en: `\n\nI also found ${webResearch.media.length} requested media result${webResearch.media.length === 1 ? '' : 's'} below.`, es: `\n\nTambién encontré ${webResearch.media.length} resultado(s) de medios solicitados abajo.`, fr: `\n\nJ’ai aussi trouvé ${webResearch.media.length} média(s) demandé(s) ci-dessous.`, de: `\n\nUnten sind ${webResearch.media.length} angeforderte Medienresultate.` }[locale]) : '';
  return { en: `I searched public internet sources for “${webResearch.query}.”\n\n${lines}${media}`, es: `Busqué fuentes públicas de internet para “${webResearch.query}.”\n\n${lines}${media}`, fr: `J’ai cherché des sources internet publiques pour « ${webResearch.query} ».\n\n${lines}${media}`, de: `Ich habe öffentliche Internetquellen zu „${webResearch.query}“ durchsucht.\n\n${lines}${media}` }[locale];
}
...
export function composeOfflineReply({ input, state, webResearch } = {}) {
  ...
  const intent = detectOfflineIntent(input);
