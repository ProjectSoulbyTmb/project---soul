...
export function normalizeProviderEndpoint(
  endpoint,
  { localOnly = false, loopbackOnly = false } = {}
) {
  ...
  if ((localOnly || loopbackOnly) && !loopback)
    throw new Error('The local provider must use a loopback address.');
...
export function validatedEndpoint(endpoint, { localOnly = false, loopbackOnly = false } = {}) {
  return normalizeProviderEndpoint(endpoint, {
    localOnly: localOnly || loopbackOnly,
    loopbackOnly,
  });
}
...
export async function callCompatibleProvider({
  ...
  try {
    const res = await fetch(
      `${normalizeProviderEndpoint(endpoint)}${COMPATIBLE_PROVIDER_CHAT_PATH}`,
      {
        method: 'POST',
...
export async function callLocalProvider({
  endpoint = LOCAL_PROVIDER_DEFAULT_ENDPOINT,
  model,
  messages,
  timeoutMs = 120000,
}) {
  ...
  try {
    const res = await fetch(
      `${normalizeProviderEndpoint(endpoint, { localOnly: true })}${LOCAL_PROVIDER_CHAT_PATH}`,
      {
        method: 'POST',
        signal: controller.signal,
        redirect: 'error',
