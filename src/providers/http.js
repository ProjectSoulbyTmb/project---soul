...
export async function callCompatibleProvider({ endpoint, apiKey, model, messages, timeoutMs = 90000 }) {
  ...
  try {
    const res = await fetch(providerRequestUrl(validatedEndpoint(endpoint), '/chat/completions'), {
...
export async function callLocalProvider({ endpoint = 'http://127.0.0.1:11434', model, messages, timeoutMs = 120000 }) {
  ...
  const timer = setTimeout(() => controller.abort(), timeoutMs);
  try {
    const res = await fetch(providerRequestUrl(validatedEndpoint(endpoint, { localOnly: true }), '/api/chat'), {
      method: 'POST', signal: controller.signal,
      redirect: 'error',
