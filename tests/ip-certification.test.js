...
test('ip-certification inventory exists, is honest, and every listed path is present', () => {
  ...
  matchTolerant(read('docs/INFRINGEMENT.md'), /DMCA/);
  matchTolerant(read('docs/INFRINGEMENT.md'), /not legal advice/i);
  matchTolerant(read('docs/COPYRIGHT_DEPOSIT.md'), /copyright\.gov/);
  matchTolerant(read('docs/COPYRIGHT_DEPOSIT.md'), /npm run ip:deposit/);
  assert.doesNotMatch(read('docs/COPYRIGHT_DEPOSIT.md'), /Registration Number TX/i);
  matchTolerant(read('.github/ISSUE_TEMPLATE/ip-notice.md'), /DMCA/);
  matchTolerant(read('.gitignore'), /copyright-deposit\//);
  matchTolerant(read('.gitignore'), /executed-assignments\//);
  matchTolerant(read('package.json'), /"ip:deposit"/);
});
...
test('legal hub, README, and ownership point at the self-attestation without fake filings', () => {
  matchTolerant(read('docs/legal.html'), /IP_CERTIFICATION\.md/);
  matchTolerant(read('docs/legal.html'), /INFRINGEMENT\.md/);
  matchTolerant(read('docs/licensing.html'), /IP_CERTIFICATION\.md/);
  matchTolerant(read('README.md'), /IP_CERTIFICATION\.md/);
  matchTolerant(read('OWNERSHIP.md'), /IP_CERTIFICATION\.md/);
  matchTolerant(read('COPYRIGHT.txt'), /IP_CERTIFICATION\.md/);
  matchTolerant(read('docs/COPYRIGHT.md'), /IP_CERTIFICATION\.md/);
  matchTolerant(read('docs/IP_PROTECTION.md'), /IP_CERTIFICATION\.md/);
  matchTolerant(read('docs/CHAIN_OF_TITLE.md'), /IP_CERTIFICATION\.md/);
  matchTolerant(read('.github/CODEOWNERS'), /IP_CERTIFICATION\.md/);
  matchTolerant(read('docs/faq.html'), /IP_CERTIFICATION\.md/);
  const pkg = JSON.parse(read('package.json'));
  assert.ok(pkg.build.files.includes('docs/IP_CERTIFICATION.md'));
  matchTolerant(read('scripts/create-release-evidence.js'), /IP_CERTIFICATION\.md/);
});
