...
test('mediaPlayback confirm is required except after an explicit picker or when enabled', () => {
  assert.deepEqual(mediaPlaybackDecision('disabled'), {
    allowed: false,
    needsConfirm: false,
    mode: 'disabled',
  });
  assert.deepEqual(mediaPlaybackDecision('confirm'), {
    allowed: true,
    needsConfirm: true,
    mode: 'confirm',
  });
  assert.deepEqual(mediaPlaybackDecision('confirm', { alreadyConfirmed: true }), {
    allowed: true,
    needsConfirm: false,
    mode: 'confirm',
  });
  assert.deepEqual(mediaPlaybackDecision('enabled'), {
    allowed: true,
    needsConfirm: false,
    mode: 'enabled',
  });
