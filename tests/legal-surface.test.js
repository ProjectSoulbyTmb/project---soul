...
test('legal docs state source-available 18+ unsigned Windows terms without fake registrations', () => {
  ...
  assert.doesNotMatch(
    read('TERMS.md'),
    /patent pending|PCI[- ]DSS certified|federally registered trademark/i
  );
...
test('LICENSE, TERMS, and NOTICE reserve first-party rights and are not OSI open source', () => {
  ...
  for (const text of [
    ...
    eula,
    read('OWNERSHIP.md'),
    read('LEGAL_NOTICES.md'),
  ]) {
    matchTolerant(text, /Copyright .{0,6}2026 Soul Consciousness Studios\. All rights reserved/i);
    matchTolerant(text, /Source-available/i);
    matchTolerant(text, /LICENSE/);
    matchTolerant(text, /TERMS/);
    matchTolerant(text, /[Tt]hird-party/);
  }
  matchTolerant(license, /Eidovara Source-Available Evaluation License/);
  matchTolerant(license, /18 years of age or older/);
  matchTolerant(license, /does not claim ownership of Electron, Chromium, Node\.js/);
  matchTolerant(license, /Microsoft Windows/);
  matchTolerant(license, /Wikipedia\/Wikimedia/);
  matchTolerant(license, /user-created content/);
  matchTolerant(license, /intended publisher/);
  matchTolerant(license, /not a U\.S\. Copyright Office\s+registration/i);
  ...
  matchTolerant(license, /not an\s+OSI-approved/i);
  matchTolerant(license, /LicenseRef-Eidovara-Source-Available-1\.0/);
  matchTolerant(license, /relicense/);
  matchTolerant(license, /not convert first-party material into OSI open source/i);
  matchTolerant(license, /does not make the submitter a joint author/i);
  assert.doesNotMatch(license, /this is an OSI[- ]approved|OSI-approved open source license/i);
  assert.doesNotMatch(
    license,
    /Copyright Office registration no\.|U\.S\. Patent No\.|patent pending/i
  );
  matchTolerant(terms, /not MIT, Apache, or GPL/);
  matchTolerant(terms, /not.*OSI open-source/i);
...
test('ownership record is honest: GitHub ToS, user content, unsigned templates, no counsel opinion', () => {
  ...
  assert.match(cla, /Soul Consciousness Studios/);
  ...
  assert.match(
    ...
    /prior written approval|first approves the work in writing/i
  );
  assert.match(read('CONTRIBUTING.md'), /docs\/CONTRIBUTOR_ASSIGNMENT\.md/);
  assert.match(read('.github/pull_request_template.md'), /Soul Consciousness Studios/);
  ...
  assert.doesNotMatch(
    read('docs/CHAIN_OF_TITLE.md'),
    /this assignment has been signed|Copyright Office registration number/i
  );
...
test('legal-instrument pack is templates and notices, not registrations or OSI giveaway', () => {
  ...
  matchTolerant(copyright, /copyright\.gov/);
  matchTolerant(copyright, /not a U\.S\. Copyright Office registration/i);
  matchTolerant(copyright, /standard file header|single block/i);
  assert.doesNotMatch(copyright, /Registration Number TX|Certificate of Registration issued/i);
  matchTolerant(filing, /not a trademark application/i);
  matchTolerant(filing, /No trademark application is filed by this commit/i);
  matchTolerant(filing, /Class/);
  matchTolerant(filing, /specimen/i);
  assert.doesNotMatch(filing, /Serial No\.|Registration No\. \d{7}/);
  matchTolerant(brand, /system font/i);
  matchTolerant(brand, /SF Pro/);
  matchTolerant(brand, /Do not/);
  matchTolerant(brand, /Jarvis|Marvel/i);
  matchTolerant(claim, /Soul Consciousness Studios/);
  matchTolerant(claim, /not a U\.S\. Copyright Office registration/i);
  matchTolerant(claim, /Eidovara Source-Available Evaluation License/);
  matchTolerant(marks, /Eidovara is a trademark of Soul Consciousness Studios \(unregistered\)/);
  matchTolerant(marks, /Windows/);
  matchTolerant(marks, /GitHub/);
  matchTolerant(marks, /Electron/);
  matchTolerant(marks, /Cloudflare/);
  matchTolerant(marks, /Wikipedia/);
  matchTolerant(marks, /Spotify/);
  matchTolerant(marks, /YouTube/);
  matchTolerant(marks, /not affiliated/i);
  matchTolerant(marks, /Marvel/);
  matchTolerant(marks, /Jarvis/i);
  matchTolerant(
    read('docs/CONTRIBUTOR_ASSIGNMENT.md'),
    /Sign privately; posting a PR is not assignment/i
  );
  matchTolerant(read('docs/legal.html'), /COPYRIGHT\.md/);
  matchTolerant(read('docs/legal.html'), /TRADEMARK_FILING\.md/);
  matchTolerant(
    read('docs/licensing.html'),
    /Eidovara is a trademark of Soul Consciousness Studios \(unregistered\)/
  );
  matchTolerant(read('src/renderer/index.html'), /LICENSE and TRADEMARKS\.md/);
  matchTolerant(read('NOTICE.md'), /Electron/);
  matchTolerant(read('NOTICE.md'), /43\.4\.1/);
  matchTolerant(read('NOTICE.md'), /rcedit/);
...
test('website legal pages cover terms, privacy, age, and Apple disclaimer', () => {
  ...
  matchTolerant(site, /Free Alpha \(18\+\)|Download Windows/);
  matchTolerant(site, /Source-available, not open source/);
  matchTolerant(site, /not an iOS or iPhone product/i);
  matchTolerant(site, /id="download"/);
  matchTolerant(site, /Get Eidovara/);
  matchTolerant(site, /Soul Consciousness Studios/);
  matchTolerant(terms, /Acceptable use/);
  matchTolerant(terms, /Wikipedia\/Wikimedia/);
  matchTolerant(privacy, /What can leave this device/);
  matchTolerant(age, /Eidovara is for adults 18+/);
  matchTolerant(age, /--i-am-18-or-older/);
  matchTolerant(licensing, /Soul Consciousness Studios/);
  matchTolerant(licensing, /Source-available; use governed by LICENSE \+ TERMS/);
  matchTolerant(licensing, /not an OSI open-source license/);
  matchTolerant(licensing, /not community OSI open source|not a formed entity/);
  for (const page of [site, terms, privacy, age, licensing]) {
    matchTolerant(page, /Content-Security-Policy/i);
    matchTolerant(page, /script-src '(?:self|none)'/);
    assert.doesNotMatch(page, /unsafe-inline|unsafe-eval/);
    matchTolerant(page, /Soul Consciousness Studios/);
    assert.doesNotMatch(
      page,
      /official iOS app|Apple Inc\. product|licensed SF Pro files are required/i
    );
...
test('desktop age gate requires terms checkbox and main-process enforcement', () => {
  ...
  matchTolerant(main, /function requireAgeGate/);
  matchTolerant(main, /requireAgeGate\(\)/);
  matchTolerant(main, /confirmed !== true/);
  matchTolerant(preload, /acceptAgeGate: confirmed/);
  matchTolerant(html, /ageGateTermsCheck/);
  matchTolerant(html, /id="legalOverlay"/);
  matchTolerant(html, /I confirm I am 18 or older and I accept the Terms/);
  matchTolerant(html, /id="ageGateAcceptBtn" disabled/);
  matchTolerant(renderer, /acceptAgeGate\(true\)/);
  matchTolerant(renderer, /ageGateTermsCheck/);
  matchTolerant(renderer, /showLegal\('terms'\)/);
  assert.doesNotMatch(html, /media-src [^"]*'self'/);
  matchTolerant(html, /media-src https: eidovara-media:/);
});

test('in-app legal overlay does not claim Apple, payments, or consciousness', () => {
  const html = read('src/renderer/index.html');
  matchTolerant(html, /not an iOS or iPhone product/);
  ...
  matchTolerant(html, /Soul Consciousness Studios™ \(unregistered\)/);
  matchTolerant(html, /LICENSE and TRADEMARKS\.md/);
  matchTolerant(html, /intended publisher only/);
  matchTolerant(html, /pull requests do not transfer ownership/i);
  matchTolerant(html, /unsigned templates only/);
...
test('network, security, and licensing docs match current fail-closed v0.19.1 surface', () => {
  ...
  assert.match(license, /Soul Consciousness Studios/);
  ...
  assert.match(
    ...
    /checkoutEnabledFromRemoteConfig\(_body\) \{\s*return false;/
  );
...
test('first-party JS carries SPDX source-available headers and does not donate OSS rights', () => {
  ...
  for (const file of files) {
    ...
    matchTolerant(head, header, file);
    matchTolerant(head, spdx, file);
    assert.doesNotMatch(head, /SPDX-License-Identifier: MIT|Apache-2\.0|GPL/);
  }
  matchTolerant(read('CONTRIBUTING.md'), /Drive-by pull requests do not create ownership/i);
  matchTolerant(
    read('CONTRIBUTING.md'),
  ...
  matchTolerant(owners, /^\* @ProjectSoulbyTmb/m);
  matchTolerant(owners, /LICENSE @ProjectSoulbyTmb/);
  ...
  matchTolerant(read('docs/legal.html'), /LicenseRef-Eidovara-Source-Available-1\.0/);
  matchTolerant(read('SECURITY.md'), /private vulnerability-reporting/);
  matchTolerant(read('docs/CONTRIBUTOR_ASSIGNMENT.md'), /template only/);
  matchTolerant(read('docs/ENTITY_IP_ASSIGNMENT.md'), /not executed/);
  assert.doesNotMatch(read('docs/CONTRIBUTOR_ASSIGNMENT.md'), /signed on |executed copy attached/i);
  assert.doesNotMatch(
    read('LICENSE') + read('OWNERSHIP.md') + read('TRADEMARKS.md'),
    /USPTO Registration No|Copyright Office registration number|U\.S\. Patent No/
  );
});
...
test('first-party legal stack is kept; third-party brands are not product names', () => {
  ...
  for (const file of [
    ...
    assert.match(read(file), /Copyright .{0,8}2026 Soul Consciousness Studios/i, file);
  }
  ...
  const identityMisuse =
    /Eidovara Jarvis|Jarvis kernel|Jarvis mode|Soul Jarvis|like Jarvis|our Jarvis|Hey Siri|OK Google|Okay Google|Hey Cortana|Eidovara (?:Raycast|Alfred|Spotlight|Copilot)/i;
...
test('no third-party brand names used as product feature names', () => {
  const bannedBrands =
    /Jarvis|Iron Man|Marvel|Disney|Stark|FRIDAY|Siri|Alexa|Google Assistant|Cortana|ChatGPT|Claude|Raycast|Alfred|Spotlight|Clippy|Replika|Character.AI|Xbox|Game Bar/;
  const filesToCheck = [
    'README.md',
    'docs/index.html',
    'docs/product.html',
  ...
  for (const file of filesToCheck) {
    const text = read(file);
    assert.doesNotMatch(text, bannedBrands, file), `Banned brand name found in ${file}`;
  }
});
...
test('no USPTO/trademark registration numbers claimed in any document', () => {
  const docs = ['LICENSE', 'TERMS.md', 'OWNERSHIP.md', 'TRADEMARKS.md', 'docs/COPYRIGHT.md'];
  for (const doc of docs) {
    const content = read(doc);
    assert.doesNotMatch(
      content,
      /USPTO Registration No\./,
      `${doc} should not claim USPTO registration`
    );
    assert.doesNotMatch(
      content,
      /Registration Number \d{6,}/,
      `${doc} should not have fake registration number`
    );
    assert.doesNotMatch(
      content,
      /Copyright Office Registration No\./,
      `${doc} should not claim Copyright Office registration`
    );
  }
});
...
test('check script validates all first-party JS syntax', () => {
  ...
  matchTolerant(check, /walk/);
  matchTolerant(check, /\.(?:js|cjs)/);
  matchTolerant(check, /--check/);
});
...
test('package.json engines pin the supported Node and pnpm lines', () => {
  const pkg = JSON.parse(read('package.json'));
...
test('AGE.md explicitly states 18+ requirement', () => {
  ...
  matchTolerant(age, /Eidovara is adult-only software/);
  matchTolerant(age, /at least 18 years old/);
  matchTolerant(age, /--i-am-18-or-older/);
  matchTolerant(age, /Local confirmation is not independent identity verification/);
});

// Additional edge case: TERMS.md commerce section is clear
test('TERMS.md commerce section is clear about no live payments', () => {
  const terms = read('TERMS.md');
  matchTolerant(terms, /This release does not process live payments/i);
  matchTolerant(terms, /Premium feature gates exist for local administrator testing only/i);
  matchTolerant(terms, /That override is not proof of purchase or subscription/i);
});
...
test('no source file predicts consciousness or sentience for Soul', () => {
  const consciousnessClaims =
    /I am conscious|scientifically proven consciousness|sentient|sentience|AI consciousness|artificial consciousness/;
  const sourceFiles = [
    ...fs
      .readdirSync('src', { withFileTypes: true })
      .filter(e => e.isFile() && e.name.endsWith('.js'))
