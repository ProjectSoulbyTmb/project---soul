...
function tmp() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'soul-coverage-'));
}
function make(dir) {
...
test('kernel routes companion coverage phrases and persists actions on the conversation', async () => {
  ...
  assert.ok(
    backups.kernel.actions.some(a => a.panel === 'backupSection' || a.type === 'open-view')
  );
...
test('optional Soul stays honest until setup; Assist never runs without opt-in', async () => {
  ...
  assert.equal(
    canCallAssist({ optIn: false, serviceUrl: 'https://api.eidovara.org' }).reason,
    'opt-in-off'
  );
  let called = 0;
...
test('desktop companion chrome handles every action type and keeps one composer', () => {
  ...
  for (const type of KERNEL_ACTION_TYPES) {
    matchTolerant(renderer, new RegExp(`action\\.type==='${type}'`));
  }
  matchTolerant(renderer, /soul\.send\(text,\s*\{\s*view:/);
  assert.doesNotMatch(renderer, /if\(stayCompanion\)\{\s*setView\('dashboard'\)/);
  matchTolerant(renderer, /e\.key==='\/'/);
  matchTolerant(renderer, /appendKernelActions/);
  matchTolerant(companion, /syncHistory/);
  matchTolerant(companion, /What can you do here/);
  matchTolerant(companion, /eidovaraActiveConversation/);
  matchTolerant(html, /id="companionFollowups"/);
  matchTolerant(html, /id="companionForm"/);
  matchTolerant(html, /id="companionInput"/);
  matchTolerant(html, /placeholder="https:\/\/api\.eidovara\.org"/);
  assert.doesNotMatch(html, /workers\.dev/);
  assert.doesNotMatch(html, /media-src [^"]*'self'/);
  matchTolerant(preload, /send: \(m, opts\)/);
  matchTolerant(read('src/electron/main.js'), /respond\(m, incoming\)/);
});

test('local-media and service copy stay inside constraints', () => {
  const st = defaultProfile();
  startKernelSession(st);
  const media = composeOfflineReply({
    input: 'Open local media',
    state: st,
    intent: 'local-media',
  });
  assert.match(media, /eidovara-media/);
  assert.doesNotMatch(media, /workers\.dev/);
  const service = composeOfflineReply({
    input: 'Open service settings',
    state: st,
    intent: 'service',
  });
