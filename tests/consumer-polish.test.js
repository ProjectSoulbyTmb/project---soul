...
test('first-run after 18+ has one Dashboard path and Assist is not Soul', () => {
  ...
  matchTolerant(html, /id="startPath"/);
  matchTolerant(html, /data-i18n="startPathTitle"/);
  matchTolerant(html, /Assist is not Soul/);
  matchTolerant(html, /id="welcomeDashboardBtn"/);
  matchTolerant(html, /You're in\. Start on the Dashboard/);
  matchTolerant(html, /id="companionForm"/);
  matchTolerant(html, /id="companionInput"/);
  matchTolerant(renderer, /setView\(settings\.ageGateAccepted\?'dashboard':'chat'\)/);
  matchTolerant(renderer, /eidovara\.startPathDismissed/);
  matchTolerant(html, /I confirm I am 18 or older/);
...
test('keyboard cheatsheet and Ctrl+K palette are discoverable without stealing Ctrl+A from fields', () => {
  const html = read('src/renderer/index.html');
  const renderer = read('src/renderer/renderer.js');
  matchTolerant(html, /id="commandPalette"/);
  matchTolerant(html, /id="shortcutSheet"/);
  matchTolerant(html, /id="paletteBtn"/);
  matchTolerant(html, /aria-keyshortcuts="Control\+K"/);
  matchTolerant(html, /aria-keyshortcuts="Control\+\/"/);
  matchTolerant(html, /Ctrl\+A is admin only when you are not typing/);
  matchTolerant(renderer, /e\.key\.toLowerCase\(\)==='k'/);
  matchTolerant(renderer, /e\.key==='\/'\|\|e\.key==='\?'/);
  matchTolerant(renderer, /input, textarea, select, \[contenteditable="true"\]/);
  matchTolerant(renderer, /openPalette/);
  matchTolerant(renderer, /openShortcutSheet/);
  matchTolerant(renderer, /function bindHoldToTalk/);
  matchTolerant(renderer, /function focusCompanion/);
});
