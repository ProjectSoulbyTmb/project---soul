...
test('overlay HTML keeps media-src off self and the workspace renderer stays locked', () => {
  for (const file of [
    ...
    matchTolerant(html, /connect-src 'none'/, file);
  }
  matchTolerant(read('src/renderer/index.html'), /media-src https: eidovara-media:/);
  matchTolerant(read('src/renderer/index.html'), /id="overlayPanel"/);
  matchTolerant(read('src/renderer/renderer.js'), /action\.type==='open-overlay'/);
  matchTolerant(read('src/renderer/renderer.js'), /action\.type==='open-discord-overlay'/);
  matchTolerant(read('src/electron/preload.cjs'), /openOverlay:/);
  const main = read('src/electron/main.js');
  matchTolerant(main, /createGuestOverlayManager/);
  matchTolerant(main, /attachOverlayWindows/);
  ...
  matchTolerant(guest, /nodeIntegration: false/);
  matchTolerant(guest, /will-navigate/);
  const createGuestBody = guest.slice(
    guest.indexOf('function createGuest(kind'),
    guest.indexOf('function wirePair')
  );
  matchTolerant(createGuestBody, /partition: opts\.partition/);
