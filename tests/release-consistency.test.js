...
test('one canonical source version across package, desktop kernel, site helper, and API', () => {
  ...
  assert.equal(pkg.version, SOURCE_VERSION, 'package.json must match src/core/release.js SOURCE_VERSION');
  assert.equal(DESKTOP_KNOWLEDGE_VERSION, SOURCE_VERSION, 'desktop kernel must follow release.js');
  assert.equal(ASSIST_VERSION, SOURCE_VERSION, 'docs/knowledge.js ASSIST_VERSION must follow release.js');
});

test('worker advertises exactly the canonical installer metadata', () => {
  assert.equal(WORKER_LIVE_INSTALLER_VERSION, LIVE_INSTALLER_VERSION, 'worker LIVE_INSTALLER_VERSION drifted from release.js');
  assert.equal(WORKER_LIVE_INSTALLER, INSTALLER_NAME, 'worker installer filename drifted from release.js');
  assert.equal(WORKER_LIVE_INSTALLER_SHA256, INSTALLER_SHA256, 'worker installer SHA-256 drifted from release.js');
  assert.equal(WORKER_LIVE_INSTALLER_SIZE, INSTALLER_SIZE_BYTES, 'worker installer size drifted from release.js');
  assert.equal(WORKER_LIVE_INSTALLER_URL, INSTALLER_LATEST_URL, 'worker installer URL drifted from release.js');
});
...
test('release pipeline derives artifact names from the version instead of frozen strings', () => {
  ...
  assert.match(pkgRaw, /"artifactName": "Eidovara-v\$\{version\}-Windows-\$\{arch\}-Setup\.\$\{ext\}"/, 'NSIS artifactName must keep the v-prefixed version template');
  assert.match(pkgRaw, /"artifactName": "Eidovara-v\$\{version\}-Windows-\$\{arch\}-Portable\.\$\{ext\}"/, 'portable artifactName must keep the v-prefixed version template');

  const manifestScript = read('scripts/create-update-manifest.js');
  assert.match(manifestScript, /Eidovara-v\$\{pkg\.version\}-Windows-x64-Setup\.exe/, 'update manifest must use the v-prefixed installer name');
