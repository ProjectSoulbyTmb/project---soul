...
test('knowledge answers stay honest and never call fetch or /v1/assist', async () => {
  ...
  try {
    ...
    assert.match(hosted.reply, /never POSTs to Worker \/v1\/assist|not sent/i);
...
test('desktop companion and send path do not POST chat to the Worker', () => {
  ...
  assert.doesNotMatch(joined, /postAssistQuery|sendToService|soulOnline/);
  assert.doesNotMatch(joined, /method:\s*['"]POST['"]/);
