...
test('desktop companion and send path do not POST chat to the Worker', () => {
  ...
  matchTolerant(html, /id="companionPanel"/);
  matchTolerant(html, /id="companionForm"/);
  matchTolerant(html, /data-companion-nav="apps"/);
  matchTolerant(html, /connect-src 'none'/);
  assert.doesNotMatch(html, /media-src [^"]*'self'/);
  assert.doesNotMatch(joined, /dreambot333\.workers\.dev/);
  matchTolerant(fs.readFileSync('src/renderer/renderer.js', 'utf8'), /e\.key==='\/'/);
  matchTolerant(fs.readFileSync('src/renderer/renderer.js', 'utf8'), /surface==='companion'/);
});
