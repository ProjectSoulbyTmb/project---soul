...
test('dashboard starters get useful offline replies instead of a generic listener', async () => {
  const s = make(tmp());
  s.configureSetup({
    categories: ['gaming-editing', 'studying', 'accessibility'],
    customNeeds: 'Keep instructions short',
  });
  ...
  const prompts = [
    ...
    [
      'Search the internet for current information I need.',
      /search the internet|wikipedia|specific topic/i,
    ],
  ...
  for (const [prompt, pattern] of prompts) {
    ...
    assert.match(
      r.reply,
      /software assistant|not a person|not a claim of consciousness|software self-model/i,
      prompt
    );
  }
});
...
test('intent detection covers workspace starters', () => {
  ...
  assert.equal(
    detectOfflineIntent('Review what you remember and suggest useful updates.'),
    'memory'
  );
  assert.equal(detectOfflineIntent('I want to talk something through with you.'), 'talk');
  assert.equal(
    detectOfflineIntent('Surprise me with lawful, properly sourced public media.'),
    'surprise'
  );
  assert.equal(
    detectOfflineIntent('Find music that fits my current mood and explain why.'),
    'mood'
  );
  assert.equal(
    detectOfflineIntent('Help me add a trusted Windows app from the Start Menu.'),
    'apps'
  );
  assert.equal(
    detectOfflineIntent('Search the internet for current information I need.'),
    'research'
  );
});
...
test('untrusted memory text is quoted and OBS URLs stay out of context and offline replies', () => {
  ...
  const reply = composeOfflineReply({
    input: 'Review what you remember and suggest useful updates.',
    state: st,
  });
...
test('invalid stream-helper OBS URLs fail with a usable error', () => {
  const s = make(tmp());
  assert.throws(
    () => s.configureSetup({ categories: ['stream-helper'], obsWebSocketUrl: 'not-a-url' }),
    /ws:\/\/ or wss:\/\//i
  );
  assert.throws(
    () =>
      s.configureSetup({ categories: ['stream-helper'], obsWebSocketUrl: 'http://127.0.0.1:4455' }),
    /ws:\/\/ or wss:\/\//i
  );
});
...
test('accessibility setup role persists and language survives a later partial behavior save', () => {
  ...
  const st = s.configureSetup({
    categories: ['accessibility', 'personal'],
    customNeeds: 'Large text',
  });
...
test('mix briefing prefers favorites and keeps HTTPS handoff copy', () => {
  ...
  recordMediaEvent(state, {
    ...
    type: 'audio',
    title: 'Harbor Light',
    sourceUrl: 'https://example.test/h',
  });
  recordMediaEvent(state, {
    event: 'skip',
    type: 'audio',
