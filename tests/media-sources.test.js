...
function tmp() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'soul-media-sources-'));
}
...
test('explicit internet lookup is not Wikipedia-only: Archive catalog plus YouTube/Spotify/Archive chips', async () => {
  ...
  try {
    ...
    assert.ok(
      r.sources.some(s => s.hostname === 'archive.org' && /Saturn recordings/.test(s.title))
    );
    assert.ok(
      hosts.some(h => h !== 'en.wikipedia.org'),
      'Wikipedia must not be the only source host'
    );
    assert.ok(
      r.handoffs.some(
        item => item.provider === 'YouTube' && /youtube\.com\/results\?search_query=/.test(item.url)
      )
    );
    assert.ok(
      r.handoffs.some(
        item => item.provider === 'Spotify' && /open\.spotify\.com\/search\//.test(item.url)
      )
    );
    assert.ok(
      r.handoffs.some(
        item =>
          item.provider === 'Internet Archive' && /archive\.org\/search\?query=/.test(item.url)
      )
    );
    assert.equal(
      seen.some(url => /youtube\.com|spotify\.com/i.test(url)),
      false
    );
...
test('mood and music requests stay offline: local library plus constructed search chips, no remote fetch', async () => {
  ...
  try {
    ...
    assert.equal(
      isMediaDiscoveryRequest('Find music that fits my current mood and explain why.'),
      true
    );
    assert.equal(
    ...
    assert.ok(
      r.mediaDiscovery.handoffs.some(
        item =>
          item.provider === 'YouTube' &&
          item.url.startsWith('https://www.youtube.com/results?search_query=')
      )
    );
    assert.ok(
      r.mediaDiscovery.handoffs.some(
        item =>
          item.provider === 'Spotify' && item.url.startsWith('https://open.spotify.com/search/')
      )
    );
    assert.ok(
      r.mediaDiscovery.handoffs.some(
        item =>
          item.provider === 'Internet Archive' &&
          item.url.startsWith('https://archive.org/search?query=')
      )
    );
    assert.match(r.reply, /Harbor Light/);
    assert.match(r.reply, /YouTube|Spotify|Internet Archive|HTTPS/);
    assert.ok(
      r.kernel.actions.some(item => item.type === 'open-external' && /youtube\.com/.test(item.url))
    );
  } finally {
    globalThis.fetch = original;
  }
...
test('named local track still matches from internetOptions.localLibrary without an internet phrase', async () => {
  ...
  try {
    ...
    assert.equal(called, 0);
    assert.ok(
      r.mediaDiscovery.local.some(
        item => item.title === 'Harbor Light' && item.url === 'eidovara-media://harbor/'
      )
    );
    assert.match(r.reply, /Harbor Light/);
  } finally {
...
test('YouTube and Spotify stay constructed search URLs; HTML is not fetched', async () => {
  ...
  await assert.rejects(
    () =>
      fetchPublicPage('https://www.youtube.com/results?search_query=Saturn', {
        fetchImpl: async () => {
          called += 1;
          throw new Error('must not fetch YouTube HTML');
        },
      }),
  ...
  await assert.rejects(
    () =>
      fetchPublicPage('https://open.spotify.com/search/Saturn', {
        fetchImpl: async () => {
          called += 1;
          throw new Error('must not fetch Spotify HTML');
        },
      }),
    /official browser searches|does not fetch their HTML/i
  );
  ...
  try {
    const r = await researchInternet(
      'Search the internet for https://open.spotify.com/search/Saturn and Saturn'
    );
    assert.equal(
      seen.some(url => /spotify\.com|youtube\.com/i.test(url)),
      false
    );
    assert.ok(
      r.handoffs.some(
        item =>
          item.url === 'https://open.spotify.com/search/Saturn' ||
          /open\.spotify\.com\/search/.test(item.url)
      )
    );
  } finally {
    globalThis.fetch = original;
