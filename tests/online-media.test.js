...
test('renderer playback URLs are only custom protocols', () => {
  ...
  assert.equal(item.quality, 'Stream');
});
...
test('media-src is never self or https and piracy tool names stay out', () => {
  ...
  const floatHtml = read('src/renderer/float-player.html');
  const main = read('src/electron/main.js');
  const renderer = read('src/renderer/now-playing.js');
  ...
  const scanned = [
    ...
    'src/renderer/now-playing.js',
    'src/renderer/float-player.js',
    'package.json'
  ].map(read).join('\n');
