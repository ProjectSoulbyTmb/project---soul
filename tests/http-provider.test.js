...
test('local endpoints require loopback HTTP(S) and strip a pasted /api/chat path', () => {
  assert.equal(
    normalizeProviderEndpoint('http://127.0.0.1:11434/api/chat/', { localOnly: true }),
    LOCAL_PROVIDER_DEFAULT_ENDPOINT
  );
  assert.equal(
    normalizeProviderEndpoint('127.0.0.1:11434', { localOnly: true }),
    LOCAL_PROVIDER_DEFAULT_ENDPOINT
  );
  assert.equal(
    normalizeProviderEndpoint('http://localhost:11434', { localOnly: true }),
    'http://localhost:11434'
  );
  assert.equal(
    normalizeProviderEndpoint('http://[::1]:11434/api/chat', { localOnly: true }),
    'http://[::1]:11434'
  );
  assert.throws(
    () => normalizeProviderEndpoint('https://example.test', { localOnly: true }),
    /loopback/
  );
  assert.throws(
    () => normalizeProviderEndpoint('http://192.168.1.5:11434', { localOnly: true }),
    /loopback/
  );
  assert.throws(
    () => normalizeProviderEndpoint('ws://127.0.0.1:11434', { localOnly: true }),
    /HTTPS|http\(s\)/i
  );
});
...
test('compatible endpoints require HTTPS except loopback and strip a pasted /chat/completions path', () => {
  assert.equal(
    normalizeProviderEndpoint('https://api.example.test/v1/chat/completions'),
  ...
  assert.throws(
    () => normalizeProviderEndpoint('https://user:pass@api.example.test/v1'),
    /credentials/
  );
  assert.throws(() => normalizeProviderEndpoint(''), /required/i);
  assert.equal(
    normalizeProviderEndpoint('https://api.example.test/v1?api-key=secret'),
    'https://api.example.test/v1'
  );
  assert.equal(
    providerRequestUrl('https://api.example.test/v1?api-key=secret', COMPATIBLE_PROVIDER_CHAT_PATH),
...
test('providerRequestUrl does not double-append chat suffixes', () => {
  assert.equal(
    providerRequestUrl('https://api.example.test/v1', COMPATIBLE_PROVIDER_CHAT_PATH),
    'https://api.example.test/v1/chat/completions'
  );
  assert.equal(
    providerRequestUrl(
  ...
  assert.equal(
    providerRequestUrl('http://127.0.0.1:11434/api/chat', LOCAL_PROVIDER_CHAT_PATH),
