...
function registerHashes() {
  ...
  const rows = [...md.matchAll(/\|\s*`([^`]+)`\s*\|[^|]*\|\s*`([A-F0-9]{64})`\s*\|/g)];
...
test('public site wires display marks, wallpapers, and OG image without CSP or token drift', () => {
  ...
  matchTolerant(brand, /\.hero-mark\s*\{/);
  matchTolerant(brand, /\.studio-mark\s*\{/);
  matchTolerant(brand, /\.brand-stage\s*\{/);
  matchTolerant(read('docs/index.html'), /data-page="home"/);
  matchTolerant(read('docs/product.html'), /data-page="product"/);
  matchTolerant(read('docs/download.html'), /data-page="download"/);
  ...
  matchTolerant(home, /class="hero-mark"/);
  matchTolerant(home, /src="eidovara-mark\.png"/);
  matchTolerant(home, /src="soul-consciousness-studios-mark\.png"/);
  matchTolerant(home, /href="download\.html"/);
  assert.doesNotMatch(home, /class="primary[^"]*"[^>]*href="[^"]+\.exe"/);

  const product = read('docs/product.html');
  matchTolerant(product, /class="brand-stage"/);

  const download = read('docs/download.html');
  matchTolerant(download, /eidovara-mark\.png/);
  matchTolerant(download, /id="ageConfirm"/);
  matchTolerant(download, /SHA256SUMS\.txt/);
  assert.doesNotMatch(download, new RegExp(INSTALLER_SHA256));
  ...
  for (const file of htmlFiles) {
    ...
    matchTolerant(html, /rel="icon"[^>]*href="eidovara-icon\.png"/, file);
    matchTolerant(html, /img-src 'self'/, file);
    matchTolerant(html, /script-src 'self'/, file);
    assert.doesNotMatch(html, /unsafe-inline|unsafe-eval/, file);
  }
  matchTolerant(read('docs/_headers'), /img-src 'self'/);
  matchTolerant(read('docs/_headers'), /script-src 'self'/);
  assert.equal(read('docs/tokens.css'), read('src/renderer/tokens.css'));
  matchTolerant(read('docs/BRAND_GUIDE.md'), /website display marks/i);
  matchTolerant(read('docs/BRAND_GUIDE.md'), /wallpapers/);
  ...
  matchTolerant(read('CHANGELOG.md'), /website display marks/);
  matchTolerant(
    read('CHANGELOG.md'),
