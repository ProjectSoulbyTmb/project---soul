...
test('packaging publishes GitHub latest.yml and ships electron-updater', () => {
  ...
  matchTolerant(read('.github/workflows/release-windows.yml'), /latest\.yml/);
  matchTolerant(read('.github/workflows/release-windows.yml'), /dist:win:installer/);
  matchTolerant(read('package.json'), /--publish never/);
  matchTolerant(read('src/electron/main.js'), /attachDesktopUpdater/);
  matchTolerant(read('src/electron/preload.cjs'), /onUpdateStatus/);
  ...
  matchTolerant(read('src/renderer/index.html'), /id="autoCheckUpdates"/);
  matchTolerant(read('src/renderer/index.html'), /id="companionCheckUpdatesBtn"/);
  matchTolerant(read('src/renderer/index.html'), /data-companion-nav="updates"/);
  matchTolerant(
    read('src/renderer/renderer.js'),
    /action\.type==='open-updates'\|\|action\.type==='check-updates'/
  );
  matchTolerant(read('src/electron/auto-update.js'), /verifyUpdateCodeSignature/);
  matchTolerant(read('src/electron/auto-update.js'), /Authenticode-unsigned/);
});
