...
function tmp() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'soul-kernel-test-'));
}
function make(dir) {
...
test('feature registry toggles modules and matches custom actions', () => {
  ...
  const extra = runtime.register({
    id: 'evening-focus',
    title: 'Evening focus',
    intents: ['focus'],
    commands: ['lights down'],
  });
  ...
  const registry = normalizeRegistry({
    ...
    customActions: [
      { label: 'Evening', command: 'Plan a focused session for my current priority.' },
    ],
  });
  ...
  assert.equal(
    matchCustomAction(
      'please Plan a focused session for my current priority. now',
...
test('voice persist uses voiceURI and honors mute against a mock speechSynthesis', () => {
  ...
  assert.deepEqual(
    listInstalledVoices(synth).map(v => v.voiceURI),
    ['urn:os:voice:one', 'urn:os:voice:two']
  );
...
test('presence looks stay first-party and freeze under reduced motion', () => {
  ...
  assert.doesNotMatch(
    PRESENCE_LOOKS.map(look => look.description).join(' '),
    /\bis alive\b|\bis conscious\b|living person/i
  );
});
  ...
  const ok = await requestSoulAssist({
    ...
    fetchImpl: async (url, init) => {
      ...
      const body = Buffer.from(
        JSON.stringify({ reply: 'Adults 18+.', soul: true, history: 'nope' })
      );
      return {
        ok: true,
        status: 200,
