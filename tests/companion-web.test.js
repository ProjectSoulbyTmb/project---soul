...
test('companion log renders research with textContent and confirm/openExternal chips', () => {
  ...
  matchTolerant(engine, /researchInternet\(text/);
  matchTolerant(engine, /citeResearchInReply/);
  matchTolerant(companion, /companion-research-source/);
  matchTolerant(companion, /snip\.textContent/);
  matchTolerant(companion, /eidovaraOpenResearch/);
  assert.doesNotMatch(companion, /innerHTML\s*=\s*source/);
  matchTolerant(
    renderer,
    /noteExchange\?\.\(text, replyText, assistNote, \{ research: res\.webResearch/
  );
  matchTolerant(renderer, /window\.confirm/);
  matchTolerant(renderer, /openExternal/);
  matchTolerant(renderer, /window\.eidovaraOpenResearch\s*=\s*openResearchLink/);
  matchTolerant(renderer, /surface === 'companion'|surface==='companion'/);
