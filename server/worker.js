...
const LIVE_INSTALLER_SHA256 = 'A26B8232E6B81A77566610AFF110197022850AB4348F86D390663831584B5DEE';
const LIVE_INSTALLER_SIZE = 106691429;
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/v1/status') return send(publicPayload({ pages: 'Official site is https://eidovara.org (Cloudflare Pages from docs/). GitHub Pages publishes the same docs/ from main.', releases: 'https://github.com/ProjectSoulbyTmb/project---soul/releases/latest', installer: LIVE_INSTALLER_URL, assist: '/v1/assist knowledge-only, no transcripts', heartbeat: 'Desktop Connect uses GET /health, /v1/config, and /v1/status. Conversations are not sent by the heartbeat.' }), { 'cache-control': 'private, no-store' });
    if (url.pathname === '/v1/config') return send({ version: ASSIST_VERSION, liveInstallerVersion: LIVE_INSTALLER_VERSION, liveInstaller: LIVE_INSTALLER, liveInstallerSha256: LIVE_INSTALLER_SHA256, liveInstallerSize: LIVE_INSTALLER_SIZE, liveInstallerUrl: LIVE_INSTALLER_URL, website: httpsUrl(env.WEBSITE_URL), store: { stripe: httpsUrl(env.STRIPE_PAYMENT_URL), paypal: httpsUrl(env.PAYPAL_PAYMENT_URL), gumroad: httpsUrl(env.GUMROAD_PRODUCT_URL) }, paymentsEnabled: false, checkoutEnabled: false, localFirst: true, conversations: false, conversationsStored: false, ageRestricted: true, minimumAge: 18, authenticodeSigned: false, officialPlatforms: ['windows-10-11-x64'], license: 'source-available-evaluation', openSource: false, premium: 'local-admin-testing-only', privacy: 'No payment-card data is accepted by this service. Conversations are not stored here. Network use is user-directed except official update/status checks. Website assist answers from an allowlisted knowledge pack and does not store transcripts.', terms: 'Use is limited to users 18 or older under the Eidovara Source-Available Evaluation License. Application launching is user-confirmed local Windows apps; internet research and media/provider features are user-directed.' }, { 'cache-control': 'public, max-age=300' });
    return response({ error: 'not_found' }, 404);
  }
};

export { httpsUrl, ENDPOINTS, LIVE_INSTALLER_VERSION, LIVE_INSTALLER, LIVE_INSTALLER_SHA256, LIVE_INSTALLER_SIZE, LIVE_INSTALLER_URL };
