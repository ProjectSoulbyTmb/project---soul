...
| Optional Worker | Working when deployed | `/health`, `/v1/config`, `/v1/status`, and `/v1/assist` fail-closed; payments empty; Settings Connect + launch retry after 18+ against `https://api.eidovara.org` by default; website assist is paste/save-only; not required to run the app; no `workers.dev` hard-code. Redeploy with `npx wrangler deploy` after merge so the live Worker does not drift. Neural TTS/VRM/OBS/live payments stay documented only. |
...
| Windows `v0.18.2` Release | Push tag `v0.18.2` on current `main` (do not move `v0.18.0` or `v0.18.1`). `Release Windows` publishes unsigned Setup.exe. Site CTAs use `releases/latest/download/Eidovara-0.18.2-Windows-x64-Setup.exe`. |
...
| Stable public API | Custom hostname `https://api.eidovara.org` is the baked desktop default. Keep `server/worker.js` deployed with Wrangler, monitor `/health`. Override in Settings if you operate another HTTPS base. Not auto-deployed from this repository. |
