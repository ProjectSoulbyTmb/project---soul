...
| Media player and DJ helpers | Working | Persistent now-playing bar, expandable video stage, local `eidovara-media://` playback of user-selected files, confirm-first playback unless the user just picked a file, queue controls, persistent taste signals, favorites, similar-search prompts, and consent-based Spotify/YouTube HTTPS handoff (browser only; no in-app embed). |
