...
| Optional Worker | Working when deployed and a URL is saved | `/health`, `/v1/health`, `/v1/config`, `/v1/status`, and `/v1/assist` fail-closed; payments empty; Settings Connect + launch retry after 18+; website assist is paste-only; desktop assist is paste + explicit opt-in; not required to run the app; not hard-coded. Redeploy with `npx wrangler deploy` after merge so the live Worker does not drift. Neural TTS/VRM/OBS/live payments stay documented only. |
