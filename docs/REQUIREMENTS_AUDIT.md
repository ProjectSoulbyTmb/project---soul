...
| Public Windows download | Working | Primary CTA is `releases/latest/download/Eidovara-0.18.2-Windows-x64-Setup.exe` (official unsigned NSIS `.exe`, 18+ gate on Download). `/releases/latest` remains the notes/checksums page. Source repo is secondary. Tag workflow on `windows-latest`; `workflow_dispatch` uploads unsigned artifacts only. `v0.18.0` and `v0.18.1` remain published and must not be moved. |
