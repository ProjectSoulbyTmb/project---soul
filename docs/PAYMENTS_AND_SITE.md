...
4. **GitHub Releases** — Push a `v*` tag on `main` (for example `v0.19.0`) so `Release Windows` builds unsigned NSIS on `windows-latest` and publishes `Eidovara-*-Windows-x64-Setup.exe`. Tags `v0.18.0`, `v0.18.1`, `v0.18.2`, and `v0.18.3` already exist and must not be force-moved. `workflow_dispatch` on a branch only uploads unsigned build artifacts; it does not Authenticode-sign and does not publish a Release unless the ref is a `v*` tag.
