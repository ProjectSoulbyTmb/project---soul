...
Eidovara v0.18.2 is a **local-first Windows desktop app**. Making it “online for all users” means a public HTTPS site, a public Windows download, and a public Worker for `/health`, `/v1/config`, `/v1/status`, and website-helper `/v1/assist`. It does **not** mean hosting everyone’s Soul, chat, or memories in the cloud. Free / Offline Soul keeps working if the service is down. The desktop app defaults to `https://api.eidovara.org` for status/config while remaining local-first. Ask Eidovara on GitHub Pages and eidovara.org works with zero secrets.
...
| Cloudflare Worker (`npx wrangler deploy` from `server/`) | Public `/health` `/v1/config` `/v1/status` `/v1/assist`. Desktop default is `https://api.eidovara.org`. Still no `workers.dev` host is compiled into the app or public JS. Redeploy after `worker.js` changes so the live Worker does not drift. |
| Desktop app | Settings → Eidovara service **Connect** after 18+; launch retry; Ctrl+A **Soul HTTPS service** / **Test service**. Official default baked; paste another HTTPS base to override. Offline fallback. |
...
- Ask Eidovara is a website helper (`docs/knowledge.js`, `docs/assist.js`) with `script-src 'self'`. No API key. Optional Worker assist is visitor-paste/save only.
- Do not put `workers.dev` URLs in the public site as a required API. Operator-example `workers.dev` URLs belong in this owner guide only. Status prefills `https://api.eidovara.org`. The helper fails closed when no base is saved.
...
4. Copy an override HTTPS **base** URL into Eidovara **Settings → Eidovara service** only if you are not using the official default. After the 18+ gate the app requests `{base}/health`, `{base}/v1/config`, and `{base}/v1/status` against `https://api.eidovara.org` unless overridden. Health JSON is `{ "service": "Eidovara", "status": "ok", "version": "0.18.2" }`. If those requests fail, Offline Soul continues locally. The public site’s Ask Eidovara widget can `POST {base}/v1/assist` after a visitor saves a base; if that fetch fails, the on-page knowledge pack still answers.
...
The desktop product and the public HTML/JS never hard-code `workers.dev`. Users do not need a `workers.dev` URL. Ask Eidovara on eidovara.org and GitHub Pages works with zero secrets via `docs/knowledge.js`.

Official baked default (overridable): `https://api.eidovara.org`. Operator `workers.dev` example (not compiled into the app or public site): `https://eidovara-api.dreambot333.workers.dev` currently also serves `/health`, `/v1/config`, `/v1/status`, and `/v1/assist` on Workers Free **after** you re-deploy `worker.js` from `main` (`npx wrangler deploy`). A live Worker can drift behind git until that command runs.
...
1. Keep `eidovara-api` off the marketing hostname. Custom hostname `https://api.eidovara.org` is the baked desktop default.
2. Settings → Eidovara service or Ctrl+A **Test service** still accepts another HTTPS **base** (no path) as an override. Empty/default resolves to `https://api.eidovara.org`. Do not compile a `workers.dev` host.
