...
Official advertised distribution is an **Authenticode-unsigned** Windows 10/11 x64 NSIS installer for users **18+**. GitHub Actions on `windows-latest` can build that installer and publish checksums, an SPDX SBOM, `CODE-SIGNING-STATUS.txt` (unsigned), GitHub/Sigstore **build provenance**, `latest.yml` (SHA-512 for `electron-updater`), and `update.json`. That provenance is **not** Authenticode. CI cannot mint a signed Setup.exe until a code-signing identity exists outside this repository.
...
4. Until a real `Eidovara-0.19.2-Windows-x64-Setup.exe` exists, keep the public site primary download on the live hashed `v0.19.1` asset: `https://github.com/ProjectSoulbyTmb/project---soul/releases/latest/download/Eidovara-0.19.1-Windows-x64-Setup.exe`. Keep `/releases/latest` as a checksums/notes link. Do not make the GitHub source tree the main download button. Independently hashed SHA-256: `72F4D09ADA17593F0391438A5375ABC9351041DA8ABB252E68271B8FDACCA7D8` (106277155 bytes). **Do not** move tags `v0.18.0`, `v0.18.1`, `v0.18.2`, `v0.18.3`, `v0.19.0`, or `v0.19.1`. After this updater merge, tag **`v0.19.2`**. Upload `latest.yml` and `*.blockmap` on that tagged Windows job.
...
Attach `dist/Eidovara-*-Windows-x64-Setup.exe`, `dist/latest.yml`, `dist/*.blockmap`, `dist/update.json`, `dist/SHA256SUMS.txt`, `dist/SBOM.spdx.json`, and `dist/CODE-SIGNING-STATUS.txt` to a GitHub Release for the matching `v*` tag. Never upload a private signing key. Installed copies check GitHub Releases, verify checksums, and prompt before applying an update. Builds stay Authenticode-unsigned.

NSIS Setup overwrites an existing Eidovara program install (`build/installer.nsh`: `SetOverwrite on`, close/kill a running Eidovara process). App data is kept (`deleteAppDataOnUninstall: false`).
