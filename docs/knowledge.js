...
export const ENTRIES = [
  ...
  },
  {
    ...
    reply: `Official advertised installers are Authenticode-unsigned. That is intentional for v0.19.0, not a claim that a signed or Microsoft-certified build exists. We cannot Authenticode-sign until the owner provides a code-signing certificate. Windows SmartScreen may warn. Confirm 18+ on the Download page, then get ${INSTALLER_NAME} (${INSTALLER_SIZE}) from the official GitHub Releases asset and verify SHA-256 ${INSTALLER_SHA256}. GitHub/Sigstore provenance is not Authenticode. No software is perfectly secure.`,
  ...
  },
  {
    ...
    reply: 'v0.19.0 does not sell Premium and does not process payments. There is no live checkout on this website or in the app. Free is $0. Premium feature gates exist for local administrator testing only (Ctrl+A). That override is not payment or PCI processing. Payment-card data is not accepted. Store URLs on the optional Worker stay empty.',
  ...
  },
  {
    ...
    reply: 'v0.19.0 does not bundle neural TTS, VRM, MakeHuman, or OBS websocket control. It does not claim scientific consciousness. It does not offer an official Linux or macOS product, Authenticode signing, or live payments. Adult Mode is a separate triple gate and stays off by default.',
...
const REFUSALS = {
  empty: 'Ask a short product question about Eidovara v0.19.0 (download, 18+, local desktop use, payments, or legal pointers).',
