...
**Public online path:** anyone can open the HTTPS site at `https://eidovara.org/` (Home, Product, Download, Assist, Help, FAQ, Status, Legal), download the official Authenticode-unsigned Windows installer `Eidovara-0.18.2-Windows-x64-Setup.exe` (18+) from GitHub Releases, or build from source with `npm run dist:win:installer` on Windows. That `.exe` is the advertised download — not the GitHub source tree. Ask Eidovara on the site is a website helper over a fixed knowledge pack (no API key); it is not Soul. The desktop app is the product and stays local-first; it can attach to the online Eidovara service for status/config (`/health`, `/v1/config`, `/v1/status`) while remaining usable offline. That attachment is not required. Cloudflare Pages publishes the same `docs/` to eidovara.org; GitHub Pages also publishes `docs/` from `main` (PR #10 merged). Owner clicks that git cannot finish (Dependency graph, `www` DNS, `npx wrangler deploy` after Worker edits) are in [docs/PAYMENTS_AND_SITE.md](docs/PAYMENTS_AND_SITE.md) and [LIVE.md](LIVE.md).
