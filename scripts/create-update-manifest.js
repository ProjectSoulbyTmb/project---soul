...
const candidates = [`Eidovara-${pkg.version}-Windows-x64-Setup.exe`, `Eidovara-${pkg.version}-Windows-x64-Ready.zip`];
...
const manifest = { version: pkg.version, url: `https://github.com/${repository}/releases/download/v${pkg.version}/${name}`, sha256, packageType: name.endsWith('.zip') ? 'ready-folder-zip' : 'installer', notes: `Eidovara ${pkg.version}` };
