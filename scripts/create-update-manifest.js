...
const candidates = [`Project-Soul-Studios-${pkg.version}-Windows-x64-Setup.exe`, `Project-Soul-Studios-${pkg.version}-Windows-x64-Ready.zip`];
...
const manifest = { version: pkg.version, url: `https://github.com/${repository}/releases/download/v${pkg.version}/${name}`, sha256, packageType: name.endsWith('.zip') ? 'ready-folder-zip' : 'installer', notes: `Project Soul Studios ${pkg.version}` };
