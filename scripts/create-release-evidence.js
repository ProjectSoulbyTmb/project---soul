...
const privacy = { schemaVersion: 1, product: 'Eidovara', version: pkg.version, declarationType: 'self-declared-and-build-attested', telemetry: false, advertising: false, automaticExternalSafetyReporting: false, localConversationStorage: true, secretsExposedToRenderer: false, updater: { automaticCheck: true, downloadRequiresUserApproval: true, httpsRequired: true, sha256Required: true }, documents: ['PRIVACY.md', 'SECURITY.md', 'LEGAL_NOTICES.md', 'docs/NETWORK_USAGE.md'] };
fs.writeFileSync(path.join('dist', 'PRIVACY-DECLARATION.json'), `${JSON.stringify(privacy, null, 2)}\n`, 'utf8');
fs.copyFileSync('PRIVACY.md', path.join('dist', 'PRIVACY.md')); fs.copyFileSync('LEGAL_NOTICES.md', path.join('dist', 'LEGAL-NOTICES.md')); fs.copyFileSync('SECURITY.md', path.join('dist', 'SECURITY.md')); fs.copyFileSync(path.join('docs', 'NETWORK_USAGE.md'), path.join('dist', 'NETWORK-USAGE.md'));
