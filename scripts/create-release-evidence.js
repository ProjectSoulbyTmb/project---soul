...
const names = fs
  ...
  .filter(
    name =>
      /\.(?:exe|zip|json|yml)$/i.test(name) &&
      (name.startsWith('Eidovara-') || name === 'update.json' || name === 'latest.yml')
  )
  .sort();
if (!names.some(name => /Setup\.exe$/i.test(name)))
  throw new Error('The Windows setup installer is missing.');
...
const sbom = {
  ...
  creationInfo: {
    ...
    creators: ['Tool: Eidovara-release-evidence'],
  },
  packages: [
    {
      name: pkg.name,
      ...
      licenseDeclared: 'LicenseRef-Eidovara-Source-Available-1.0',
      copyrightText: 'Copyright (c) 2026 Soul Consciousness Studios. All rights reserved.',
    },
  ],
  files: files.map((f, i) => ({
    fileName: f.name,
...
const privacy = {
  ...
  product: 'Eidovara',
  version: pkg.version,
  declarationType: 'self-declared-and-build-attested',
  telemetry: false,
  advertising: false,
  automaticExternalSafetyReporting: false,
  localConversationStorage: true,
  secretsExposedToRenderer: false,
  updater: {
    automaticCheck: true,
    autoCheckDefaultOn: true,
  ...
  documents: [
    ...
    'LEGAL_NOTICES.md',
    'OWNERSHIP.md',
    'docs/NETWORK_USAGE.md',
  ],
};
fs.writeFileSync(
  path.join('dist', 'PRIVACY-DECLARATION.json'),
  `${JSON.stringify(privacy, null, 2)}\n`,
  'utf8'
);
fs.copyFileSync('PRIVACY.md', path.join('dist', 'PRIVACY.md'));
fs.copyFileSync('TERMS.md', path.join('dist', 'TERMS.md'));
fs.copyFileSync('AGE.md', path.join('dist', 'AGE.md'));
fs.copyFileSync('LEGAL_NOTICES.md', path.join('dist', 'LEGAL-NOTICES.md'));
fs.copyFileSync('SECURITY.md', path.join('dist', 'SECURITY.md'));
fs.copyFileSync(path.join('docs', 'NETWORK_USAGE.md'), path.join('dist', 'NETWORK-USAGE.md'));
for (const name of [
  'LICENSE',
  'NOTICE.md',
  'COPYRIGHT.txt',
...
fs.copyFileSync(
  path.join('docs', 'BRAND_ASSET_POLICY.md'),
  path.join('dist', 'BRAND-ASSET-POLICY.md')
);
fs.copyFileSync(
  path.join('docs', 'COPYRIGHT_ASSET_REGISTER.md'),
  path.join('dist', 'COPYRIGHT-ASSET-REGISTER.md')
);
...
fs.copyFileSync(
  path.join('docs', 'COPYRIGHT_DEPOSIT.md'),
  path.join('dist', 'COPYRIGHT-DEPOSIT.md')
);
