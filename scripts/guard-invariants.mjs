...
const checks = [
  ...
  ['tests/known-failures.json parses', () => { try { return Array.isArray(JSON.parse(fs.readFileSync('tests/known-failures.json','utf8'))); } catch { return false; } }],
  ["ci.yml wires test:gate", () => fs.readFileSync('.github/workflows/ci.yml','utf8').includes('pnpm run test:gate')],
  ['ci.yml has lockfile sync guard', () => fs.readFileSync('.github/workflows/ci.yml','utf8').includes('Lockfile sync guard')],
...
for (const [label, ok] of checks) {
  let pass = false;
  try { pass = ok(); } catch (e) { console.error(`FAIL ${label}: ${e.message}`); bad++; continue; }
  if (!pass) { console.error(`FAIL ${label}`); bad++; } else console.log(`ok   ${label}`);
}
